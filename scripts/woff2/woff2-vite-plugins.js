...
/**
 * Custom vite plugin for auto-prefixing `EXCALIDRAW_ASSET_PATH` woff2 fonts in `codenora-app`.
 *
...
module.exports.woff2BrowserPlugin = () => {
  ...
  return {
    ...
    transform(code, id) {
      ...
      // but at the same time can't be easily prefixed with the `EXCALIDRAW_ASSET_PATH` only for the `codenora-app`
      if (!isDev && id.endsWith("/excalidraw/fonts/fonts.css")) {
        return `/* WARN: The following content is generated during codenora-app build */
      ...
      if (!isDev && id.endsWith("codenora-app/index.html")) {
        return code.replace(
