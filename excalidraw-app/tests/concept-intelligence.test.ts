...
import { createSceneGraphFromActions, createSceneState } from "../ai/scene-state";
...
describe("Universal Concept Intelligence: Domain Resolution & Strategy across 12 domains", () => {
  const testCases: Array<{
  ...
  }> = [
    {
      ...
      prompt: "Insert a node with value 42 into a singly linked list after node 15",
    ...
    },
    {
      ...
      prompt: "Breadth-first search traversal on a binary tree with queue tracking",
      expectedDomain: "data_structures",
    ...
    },
    {
      ...
      prompt: "HTTP GET request lifecycle from client browser to web server and 200 OK response",
      expectedDomain: "networking",
      expectedStrategy: "STATE_MACHINE",
    },
    {
      name: "8. SQL JOIN",
      prompt: "INNER JOIN between users table and orders table on users.id = orders.user_id",
      expectedDomain: "databases",
      expectedStrategy: "DATA_FLOW",
    },
    {
      name: "9. OS Process Scheduling",
      prompt: "Round Robin CPU process scheduling with ready queue and context switching",
      expectedDomain: "operating_systems",
      expectedStrategy: "STATE_MACHINE",
    },
    {
      name: "10. Machine Learning Gradient Descent",
      prompt: "Gradient descent optimization minimizing loss function on parameter weights",
      expectedDomain: "machine_learning",
      expectedStrategy: "ITERATIVE_OPTIMIZATION",
    },
    {
      name: "11. Physics Kinematics / Trajectory",
      prompt: "Projectile motion trajectory under gravity with velocity vectors",
      expectedDomain: "physics",
      expectedStrategy: "TEMPORAL_PROGRESSION",
    },
    {
      name: "12. Generic / Unseen: Refrigerator Cycle",
      prompt: "Explain the refrigeration cycle: compressor, condenser, expansion valve, evaporator",
...
describe("Universal Concept Intelligence: Invariant and Misconception Mapping", () => {
  it("provides specific invariants and misconceptions for DSA", () => {
    ...
    expect(invs.some((i) => i.rule.includes("BST invariant") || i.rule.includes("Node identity") || i.description.includes("BST"))).toBe(true);
  ...
  it("provides specific invariants and misconceptions for Networking", () => {
    ...
    expect(invs.some((i) => i.rule.includes("Client and server") || i.rule.includes("HTTP") || i.description.includes("packet"))).toBe(true);

    const mis = NetworkingDomainModule.getMisconceptions("TCP Handshake");
    expect(mis.some((m) => (m.misconception || m.misunderstanding || "").toLowerCase().includes("handshake") || (m.misconception || m.misunderstanding || "").toLowerCase().includes("connection"))).toBe(true);
  });

  it("provides specific invariants and misconceptions for Databases", () => {
    const invs = DatabasesDomainModule.getInvariants("SQL JOIN");
    expect(invs.some((i) => i.rule.includes("Table schemas") || i.rule.includes("Join predicate") || i.description.includes("join"))).toBe(true);

    const mis = DatabasesDomainModule.getMisconceptions("INNER JOIN");
    expect(mis.some((m) => (m.misconception || m.misunderstanding || "").includes("Cartesian") || (m.misconception || m.misunderstanding || "").includes("LEFT JOIN"))).toBe(true);
  });
  ...
  it("provides specific invariants and misconceptions for Operating Systems", () => {
    ...
    expect(invs.some((i) => i.rule.includes("CPU") || i.rule.includes("Process state") || i.description.includes("Process"))).toBe(true);
  ...
  it("provides specific invariants and misconceptions for Machine Learning", () => {
    ...
    expect(invs.some((i) => i.rule.includes("Loss") || i.rule.includes("Learning rate") || i.description.includes("Loss"))).toBe(true);

    const mis = MachineLearningDomainModule.getMisconceptions("Gradient Descent");
    expect(mis.length).toBeGreaterThan(0);
  });
  ...
  it("provides specific invariants and misconceptions for Physics", () => {
    ...
    expect(invs.some((i) => i.rule.includes("Conservation") || i.rule.includes("Continuity") || i.description.includes("Momentum"))).toBe(true);
  ...
  it("gracefully provides generalized invariants and misconceptions for unseen concepts", () => {
    const invs = GenericDomainModule.getInvariants("Refrigerator Vapor-Compression");
    expect(invs.length).toBeGreaterThan(0);
    expect(invs.some((i) => i.rule.includes("Conservation") || i.rule.includes("Identity") || i.description.includes("Conservation"))).toBe(true);
...
describe("Universal Concept Intelligence: Dynamic Inspector Data Extraction (Zero UI Hardcoding)", () => {
  it("dynamically extracts inspector metrics for DSA (AVL / Binary Search)", () => {
    ...
    const data = DsaDomainModule.extractInspectorData(state as any, "AVL Balance Check" as any, 1 as any);
    expect(data.title || data.subtitle).toBeTruthy();
    const properties = (data as any).sections ? (data as any).sections.flatMap((s: any) => s.properties) : data.properties || [];
  ...
  it("dynamically extracts inspector metrics for Networking (HTTP / TCP)", () => {
    ...
    const data = NetworkingDomainModule.extractInspectorData(state as any, "Send HTTP Request" as any, 2 as any);
    expect(data.title || data.subtitle).toBeTruthy();
    const properties = (data as any).sections ? (data as any).sections.flatMap((s: any) => s.properties) : data.properties || [];
    expect(properties.length).toBeGreaterThan(0);
  });
  ...
  it("dynamically extracts inspector metrics for Databases (SQL JOIN)", () => {
    ...
    const data = DatabasesDomainModule.extractInspectorData(state as any, "Execute Hash Match" as any, 2 as any);
    expect(data.title || data.subtitle).toBeTruthy();
    const properties = (data as any).sections ? (data as any).sections.flatMap((s: any) => s.properties) : data.properties || [];
    expect(properties.length).toBeGreaterThan(0);
  });
  ...
  it("dynamically extracts inspector metrics for Operating Systems (CPU Scheduling)", () => {
    ...
    const data = OperatingSystemsDomainModule.extractInspectorData(state as any, "Context Switch to P1" as any, 3 as any);
    expect(data.title || data.subtitle).toBeTruthy();
    const properties = (data as any).sections ? (data as any).sections.flatMap((s: any) => s.properties) : data.properties || [];
    expect(properties.length).toBeGreaterThan(0);
  });
  ...
  it("dynamically extracts inspector metrics for Machine Learning (Gradient Descent)", () => {
    ...
    const data = MachineLearningDomainModule.extractInspectorData(state as any, "SGD Step 15: update weights" as any, 15 as any);
    expect(data.title || data.subtitle).toBeTruthy();
    const properties = (data as any).sections ? (data as any).sections.flatMap((s: any) => s.properties) : data.properties || [];
    expect(properties.length).toBeGreaterThan(0);
  });
  ...
  it("dynamically extracts inspector metrics for Physics (Kinematics / Newton)", () => {
    ...
    const data = PhysicsDomainModule.extractInspectorData(state as any, "Time t=2.0s: Apex reached" as any, 4 as any);
    expect(data.title || data.subtitle).toBeTruthy();
    const properties = (data as any).sections ? (data as any).sections.flatMap((s: any) => s.properties) : data.properties || [];
    expect(properties.length).toBeGreaterThan(0);
  });
  ...
  it("dynamically extracts inspector metrics for Generic / Unseen concept", () => {
    ...
    const data = GenericDomainModule.extractInspectorData(state as any, "Refrigerant compression" as any, 1 as any);
    expect(data.title || data.subtitle).toBeTruthy();
    const properties = (data as any).sections ? (data as any).sections.flatMap((s: any) => s.properties) : data.properties || [];
    expect(properties.length).toBeGreaterThan(0);
  });
...
describe("Universal Concept Model Compilation & Anti-Fake-Step Validation", () => {
  it("compiles a 4-step ConceptModel into a valid VisualLesson with non-fake steps", () => {
    const model: any = {
      ...
      strategy: "PIPELINE",
      entities: [
        { id: "comp", name: "Compressor", type: "Component", properties: { state: "compression" }, visualRole: "component" },
        { id: "cond", name: "Condenser", type: "Component", properties: { state: "heat_reject" }, visualRole: "component" },
        { id: "valve", name: "Expansion Valve", type: "Component", properties: { state: "throttling" }, visualRole: "component" },
        { id: "evap", name: "Evaporator", type: "Component", properties: { state: "heat_absorb" }, visualRole: "component" },
      ],
      relationships: [
        { id: "pipe1", from: "comp", to: "cond", type: "FLOW", label: "High-pressure vapor" },
        { id: "pipe2", from: "cond", to: "valve", type: "FLOW", label: "High-pressure liquid" },
        { id: "pipe3", from: "valve", to: "evap", type: "FLOW", label: "Low-pressure liquid/vapor" },
        { id: "pipe4", from: "evap", to: "comp", type: "FLOW", label: "Low-pressure vapor" },
      ],
      invariants: [
        { id: "inv-energy", name: "Energy Conservation", rule: "Q_in + W = Q_out", severity: "error" },
      ],
      misconceptions: [
        { id: "mis-cold", misconception: "Refrigerators create coldness", correction: "They remove heat via phase transition" },
      ],
      states: [
        { id: "s0", stepIndex: 0, title: "Initial Idle State", entityStates: {}, activeEntityIds: ["comp"] },
        { id: "s1", stepIndex: 1, title: "Compression", entityStates: {}, activeEntityIds: ["comp"] },
        { id: "s2", stepIndex: 2, title: "Condensation", entityStates: {}, activeEntityIds: ["cond"] },
        { id: "s3", stepIndex: 3, title: "Evaporation", entityStates: {}, activeEntityIds: ["evap"] },
      ],
      transformations: [
        {
          id: "t0",
          ...
          operations: [],
          visualActions: [
            { type: "create_box", id: "comp", label: "Compressor", role: "component" },
            { type: "create_box", id: "cond", label: "Condenser", role: "component" },
            { type: "create_box", id: "valve", label: "Valve", role: "component" },
            { type: "create_box", id: "evap", label: "Evaporator", role: "component" },
          ],
        },
        {
          id: "t1",
          ...
          observation: "Compressor highlights in warning amber; refrigerant enthalpy spikes",
          consequence: "High pressure gas flows to condenser",
          operations: [{ type: "UPDATE_ENTITY", entityId: "comp", properties: { temp: "high", press: "high" } }],
          visualActions: [
        ...
        },
        {
          ...
          observation: "Refrigerant transitions from vapor to liquid; Condenser activates",
          consequence: "Refrigerant liquid enters expansion valve",
          operations: [{ type: "UPDATE_ENTITY", entityId: "cond", properties: { phase: "liquid" } }],
          visualActions: [
        ...
        },
        {
          ...
          action: "Low-pressure liquid absorbs heat Q_L from refrigerator interior",
          reason: "Boils at low temperature, cooling the cabinet",
          observation: "Evaporator glows success green; cabinet temperature drops",
          consequence: "Refrigerant vaporizes and returns to compressor",
          operations: [{ type: "UPDATE_ENTITY", entityId: "evap", properties: { phase: "vapor", temp: "cold" } }],
          visualActions: [
    ...
    expect(lesson.transformations[0].explanation).toContain("Raises boiling temperature above ambient");
    ...
    const timelineVal = validateTransformationTimeline(compiled, { prompt: "Explain refrigeration cycle" });
    expect(timelineVal.valid).toBe(true);

    for (let i = 0; i < compiled.states.length - 1; i++) {
      const diff = computeSemanticDiff(compiled.states[i], compiled.states[i + 1]);
  ...
  it("detects and rejects duplicate/fake consecutive steps", () => {
    const graph1 = createSceneGraphFromActions([
      { type: "create_box", id: "node-1", label: "Node 1", style: { color: "primary" } },
    ]);
  ...
  it("extracts ConceptModel from a canonical VisualLesson faithfully", () => {
    const lesson = {
      ...
      steps: [],
      initialScene: [
        { type: "create_box" as const, id: "client", label: "Client Browser", role: "actor" as const },
        { type: "create_box" as const, id: "server", label: "Web Server", role: "server" as const },
      ],
      transformations: [
        {
          id: "t1",
          ...
          operations: [
            {
              ...
              relationship: { id: "rel-syn", from: "client", to: "server", label: "SYN seq=100" },
            },
          ],
          visualActions: [
            { type: "connect" as const, id: "conn-syn", from: "client", to: "server", label: "SYN seq=100" },
          ],
        },
      ],
    };
