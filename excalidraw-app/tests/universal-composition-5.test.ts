...
import {
  VisualInformationSelector,
} from "../ai/visual-reasoning/visual-information-selector";
import {
  computeOptimalRoute,
...
} from "../ai/visual-reasoning/relationship-label-planner";
import {
  computeOptimalCalloutPosition,
} from "../ai/visual-reasoning/callout-planner";
import {
  reconcileSceneState,
} from "../ai/scene-reconciler";
import {
  compileAuthoritativeTimeline,
  renderTimelineStep,
} from "../ai/transformation-timeline";
function createSemanticEntity(opts: { id: string; primitiveType?: string; label?: string; [key: string]: any }) {
  return {
    ...opts,
    id: opts.id,
...
function createSemanticRelationship(opts: { id: string; source: string; target: string; label?: string; [key: string]: any }) {
  return {
    ...opts,
    id: opts.id,
    source: opts.source,
...
function createSemanticState(opts: { id: string; name?: string; entities?: any; relationships?: any }) {
  ...
  } else if (opts.relationships && typeof opts.relationships.entries === "function") {
...
function createConfidence(value: number, level: any = "KNOWN", reason = "Verified") {
  return {
    value,
    level,
    reason,
...
describe("Cognora Universal Visual Rendering Intelligence 5.0", () => {
  ...
  describe("1. Universal Visual Hierarchy", () => {
    it("correctly infers PRIMARY hierarchy for structural and focal primitives", () => {
      ...
      expect(inferEntityHierarchy({ primitiveType: "ClientEndpoint" })).toBe("PRIMARY");
      expect(inferEntityHierarchy({ primitiveType: "ServerEndpoint" })).toBe("PRIMARY");
      expect(inferEntityHierarchy({ primitiveType: "TreeNode" })).toBe("PRIMARY");
      expect(inferEntityHierarchy({ primitiveType: "ArrayCell" })).toBe("PRIMARY");
    ...
    it("correctly infers SECONDARY hierarchy for payload and transition primitives", () => {
      expect(inferEntityHierarchy({ primitiveType: "MessagePacket" })).toBe("SECONDARY");
      expect(inferEntityHierarchy({ primitiveType: "TrajectoryRay" })).toBe("SECONDARY");
      expect(inferEntityHierarchy({ primitiveType: "EquationBlock" })).toBe("SECONDARY");
      expect(inferEntityHierarchy({ semanticRole: "message" })).toBe("SECONDARY");
    });

    it("correctly infers EXPLANATORY hierarchy for annotations and callouts", () => {
      expect(inferEntityHierarchy({ primitiveType: "Annotation" })).toBe("EXPLANATORY");
      expect(inferEntityHierarchy({ primitiveType: "Callout" })).toBe("EXPLANATORY");
      expect(inferEntityHierarchy({ semanticRole: "callout" })).toBe("EXPLANATORY");
    });
  ...
  describe("2. Text Sanitization & Semantic Compression", () => {
    it("sanitizes escaped HTML, control characters, and internal IDs", () => {
      const dirty = "&lt;div class=&quot;header&quot;&gt;Hello &amp; Welcome&lt;/div&gt;";
    ...
    it("intelligently truncates long general text with ellipsis", () => {
      const longText = "This is an extremely long relationship description that should never stretch across canvas";
  ...
  describe("3. Visual Information Selector", () => {
    it("selects primary entities for canvas and suppresses transient operations", () => {
      const plan = VisualInformationSelector.select({
        entities: [
          { id: "acc-a", label: "Account A ($500)", primitiveType: "TableRecord", semanticRole: "source_account" },
          { id: "acc-b", label: "Account B ($200)", primitiveType: "TableRecord", semanticRole: "dest_account" },
          { id: "op-debit", label: "Debit $100", semanticRole: "transient_action", properties: { isOperation: true } },
        ],
        relationships: [
          { id: "rel-transfer", source: "acc-a", target: "acc-b", label: "Transfer $100" },
        ],
      });
      ...
      expect(plan.relationships.get("rel-transfer")?.shouldRenderOnCanvas).toBe(true);
    });
    ...
    it("compresses tertiary labels when scene density is high", () => {
      const plan = VisualInformationSelector.select({
        ...
        relationships: [
          { id: "r1", source: "n1", target: "n2", label: "Extremely verbose relationship information that should compress" },
        ],
        sceneDensity: 0.8,
      });
  ...
  describe("4. Obstacle-Aware Connector Router", () => {
    ...
    it("assigns distinct parallel track lanes when multiple connectors share endpoints", () => {
      ...
      const routeLane0 = computeOptimalRoute(src, tgt, [], { laneIndex: 0, totalLanes: 2 });
      const routeLane1 = computeOptimalRoute(src, tgt, [], { laneIndex: 1, totalLanes: 2 });

      // Start Y positions should be separated by at least 20px
      expect(Math.abs(routeLane0.startY - routeLane1.startY)).toBeGreaterThanOrEqual(20);
    });
  });
  ...
  describe("7. Universal Spatial Property Testing across 14 Real-World Domains", () => {
    ...
    function verifySceneProperties(model: AuthoritativeSemanticModel, domainName: string) {
      ...
      for (let sIdx = 0; sIdx < timeline.states.length; sIdx++) {
        ...
        for (const el of elements) {
          if (el.customData?.isEdgeLabel && el.type === "text") {
            ...
            arrowEls.push(el);
          } else if ((el.type as string) !== "arrow" && !el.customData?.isEdgeLabel && !el.customData?.isDiagnostic) {
            primaryEls.push(el);
          }
        ...
        for (const label of labelEls) {
          ...
          for (const prim of primaryEls) {
            ...
            expect(
              overlap,
              `[${domainName} State ${sIdx}] Edge label '${(label as any).text}' overlaps node '${prim.id}'`,
            ).toBe(false);
          }
        }
        ...
        for (const el of elements) {
          expect(ids.has(el.id), `[${domainName}] Duplicate element ID: ${el.id}`).toBe(false);
          ids.add(el.id);
        }
      }
    }
    ...
    it("passes spatial properties for TCP 3-Way Handshake", () => {
      const s0 = createSemanticState({
        ...
        entities: [
          createSemanticEntity({ id: "client", primitiveType: "Client", label: "Client" }),
          createSemanticEntity({ id: "server", primitiveType: "Server", label: "Server" }),
        ],
      });
      ...
      const model: AuthoritativeSemanticModel = {
        id: "tcp-handshake",
        problem: { id: "p1", question: "TCP 3-Way Handshake", objective: "Establish TCP Connection", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0, s1, s2],
        transformations: [
          { id: "t1", title: "Client sends SYN", fromStateIndex: 0, toStateIndex: 1, explanation: "Client initiates handshake." },
          { id: "t2", title: "Server replies SYN-ACK", fromStateIndex: 1, toStateIndex: 2, explanation: "Server acknowledges SYN." },
        ],
        invariants: [],
    ...
    it("passes spatial properties for DNS Resolution", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "browser", primitiveType: "Client", label: "Browser" }),
          createSemanticEntity({ id: "resolver", primitiveType: "Server", label: "DNS Resolver" }),
          createSemanticEntity({ id: "root", primitiveType: "Server", label: "Root DNS" }),
          createSemanticEntity({ id: "tld", primitiveType: "Server", label: "TLD DNS (.com)" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "r1", source: "browser", target: "resolver", label: "Query example.com" }),
          createSemanticRelationship({ id: "r2", source: "resolver", target: "root", label: "Referral to .com" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "dns-resolution",
        problem: { id: "p2", question: "DNS Resolution Flow", objective: "Resolve IP address", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
    ...
    it("passes spatial properties for Database Transaction with Rollback", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "acc-1", primitiveType: "Record", label: "Alice: $500", value: 500 }),
          createSemanticEntity({ id: "acc-2", primitiveType: "Record", label: "Bob: $200", value: 200 }),
        ],
      });
      const s1 = createSemanticState({
        id: "s1",
        entities: [
          createSemanticEntity({ id: "acc-1", primitiveType: "Record", label: "Alice: $500", value: 500, state: "rolled_back" }),
          createSemanticEntity({ id: "acc-2", primitiveType: "Record", label: "Bob: $200", value: 200, state: "rolled_back" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "rb-flow", source: "acc-1", target: "acc-2", label: "Aborted Rollback" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "db-tx",
        problem: { id: "p3", question: "Database Balance Transfer", objective: "ACID consistency", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0, s1],
        transformations: [
          { id: "t1", title: "Rollback on Error", fromStateIndex: 0, toStateIndex: 1, explanation: "Revert changes." },
        ],
        invariants: [],
    ...
    it("passes spatial properties for AVL Tree Rotation", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "node-30", primitiveType: "TreeNode", label: "30", value: 30 }),
          createSemanticEntity({ id: "node-20", primitiveType: "TreeNode", label: "20", value: 20 }),
          createSemanticEntity({ id: "node-10", primitiveType: "TreeNode", label: "10", value: 10 }),
        ],
        relationships: [
          createSemanticRelationship({ id: "e1", source: "node-30", target: "node-20", type: "leftOf" }),
          createSemanticRelationship({ id: "e2", source: "node-20", target: "node-10", type: "leftOf" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "avl-rotation",
        problem: { id: "p4", question: "AVL Tree Right Rotation", objective: "Balance BST", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
    ...
    it("passes spatial properties for Binary Search", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "c0", primitiveType: "ArrayCell", label: "[0]=2", value: 2 }),
          createSemanticEntity({ id: "c1", primitiveType: "ArrayCell", label: "[1]=5", value: 5 }),
          createSemanticEntity({ id: "c2", primitiveType: "ArrayCell", label: "[2]=8", value: 8 }),
          createSemanticEntity({ id: "c3", primitiveType: "ArrayCell", label: "[3]=12", value: 12 }),
        ],
        relationships: [
          createSemanticRelationship({ id: "mid-pointer", source: "c0", target: "c2", label: "mid = 2" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "binary-search",
        problem: { id: "p5", question: "Binary Search", objective: "Find element in array", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
    ...
    it("passes spatial properties for Recursion Stack Frames", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "frame-3", primitiveType: "StackFrame", label: "factorial(3)" }),
          createSemanticEntity({ id: "frame-2", primitiveType: "StackFrame", label: "factorial(2)" }),
          createSemanticEntity({ id: "frame-1", primitiveType: "StackFrame", label: "factorial(1)" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "call-1", source: "frame-3", target: "frame-2", label: "calls" }),
          createSemanticRelationship({ id: "call-2", source: "frame-2", target: "frame-1", label: "calls" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "recursion-stack",
        problem: { id: "p6", question: "Factorial Call Stack", objective: "Demonstrate recursion", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.98),
    ...
    it("passes spatial properties for HTTP Lifecycle with large headers", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "client-app", primitiveType: "Client", label: "Web Client" }),
          createSemanticEntity({ id: "web-server", primitiveType: "Server", label: "API Gateway" }),
        ],
        relationships: [
          createSemanticRelationship({
            id: "http-res",
            source: "web-server",
            target: "client-app",
            label: "HTTP/1.1 200 OK Content-Type: application/json Cache-Control: max-age=3600",
          }),
        ],
      });
      ...
      const model: AuthoritativeSemanticModel = {
        id: "http-lifecycle",
        problem: { id: "p7", question: "HTTP Request Response", objective: "Explain HTTP protocol", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.97),
    ...
    it("passes spatial properties for CPU Instruction Cycle", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "pc-reg", primitiveType: "MemoryBlock", label: "PC: 0x0040" }),
          createSemanticEntity({ id: "ir-reg", primitiveType: "MemoryBlock", label: "IR: ADD R1, R2" }),
          createSemanticEntity({ id: "alu-unit", primitiveType: "ProcessNode", label: "ALU Unit" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "fetch", source: "pc-reg", target: "ir-reg", label: "Fetch" }),
          createSemanticRelationship({ id: "exec", source: "ir-reg", target: "alu-unit", label: "Decode & Exec" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "cpu-cycle",
        problem: { id: "p8", question: "CPU Instruction Cycle", objective: "Explain fetch decode execute", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.96),
    ...
    it("passes spatial properties for Photosynthesis biochemical flow", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "light-rxn", primitiveType: "ProcessNode", label: "Thylakoid Light Rxn" }),
          createSemanticEntity({ id: "calvin-cycle", primitiveType: "ProcessNode", label: "Calvin Cycle (Stroma)" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "atp-flow", source: "light-rxn", target: "calvin-cycle", label: "ATP & NADPH" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "photosynthesis",
        problem: { id: "p9", question: "Photosynthesis", objective: "Energy conversion", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.95),
    ...
    it("passes spatial properties for Projectile Motion physics", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "launcher", primitiveType: "StateNode", label: "Launch Point (x0, y0)" }),
          createSemanticEntity({ id: "apex", primitiveType: "StateNode", label: "Apex (vy=0)" }),
          createSemanticEntity({ id: "formula", primitiveType: "EquationBlock", label: "y = v0*t - 0.5*g*t^2" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "trajectory", source: "launcher", target: "apex", label: "Ascent" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "projectile-motion",
        problem: { id: "p10", question: "Projectile Motion", objective: "Kinematics", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.96),
    ...
    it("passes spatial properties for SQL JOIN relational schema", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "table-users", primitiveType: "Table", label: "Users Table" }),
          createSemanticEntity({ id: "table-orders", primitiveType: "Table", label: "Orders Table" }),
          createSemanticEntity({ id: "table-joined", primitiveType: "Table", label: "Joined Results" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "fk-join", source: "table-users", target: "table-orders", label: "users.id = orders.user_id" }),
          createSemanticRelationship({ id: "emit-res", source: "table-orders", target: "table-joined", label: "Emit Row" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "sql-join",
        problem: { id: "p11", question: "SQL INNER JOIN", objective: "Relational algebra", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.96),
    ...
    it("passes spatial properties for completely unknown invented concept", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "qubit-flock-alpha", primitiveType: "CircleNode", label: "QuFlock Alpha" }),
          createSemanticEntity({ id: "resonance-chamber", primitiveType: "Container", label: "Resonance Chamber" }),
          createSemanticEntity({ id: "flux-lattice", primitiveType: "StateNode", label: "Lattice 7B" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "entangle", source: "qubit-flock-alpha", target: "flux-lattice", label: "Phase Shift 0.3 rad" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "novel-concept",
        problem: { id: "p12", question: "QuFlock Resonance", objective: "Invented concept", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.95),
    ...
    it("passes spatial properties for Multi-Actor Distributed Consensus", () => {
      const s0 = createSemanticState({
        id: "s0",
        entities: [
          createSemanticEntity({ id: "node-leader", primitiveType: "Server", label: "Leader Node" }),
          createSemanticEntity({ id: "node-f1", primitiveType: "Server", label: "Follower 1" }),
          createSemanticEntity({ id: "node-f2", primitiveType: "Server", label: "Follower 2" }),
          createSemanticEntity({ id: "node-f3", primitiveType: "Server", label: "Follower 3" }),
        ],
        relationships: [
          createSemanticRelationship({ id: "append-1", source: "node-leader", target: "node-f1", label: "AppendEntries" }),
          createSemanticRelationship({ id: "append-2", source: "node-leader", target: "node-f2", label: "AppendEntries" }),
        ],
      });

      const model: AuthoritativeSemanticModel = {
        id: "raft-consensus",
        problem: { id: "p13", question: "Raft AppendEntries", objective: "Distributed consensus", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.97),
    ...
    it("passes spatial properties for High-Density Scene with many elements and edges", () => {
      ...
      for (let i = 1; i <= 6; i++) {
        entities.push(
          createSemanticEntity({ id: `item-${i}`, primitiveType: "GraphNode", label: `Node ${i}` }),
        );
      }
      const relationships = [
        createSemanticRelationship({ id: "e12", source: "item-1", target: "item-2", label: "e12" }),
        createSemanticRelationship({ id: "e23", source: "item-2", target: "item-3", label: "e23" }),
        createSemanticRelationship({ id: "e34", source: "item-3", target: "item-4", label: "e34" }),
        createSemanticRelationship({ id: "e45", source: "item-4", target: "item-5", label: "e45" }),
        createSemanticRelationship({ id: "e56", source: "item-5", target: "item-6", label: "e56" }),
      ];

      const s0 = createSemanticState({ id: "s0", entities, relationships });
      const model: AuthoritativeSemanticModel = {
        id: "dense-graph",
        problem: { id: "p14", question: "Dense Graph Topology", objective: "Graph layout", intent: "concept_explainer", inputs: [], outputs: [], entities: [], relationships: [], constraints: [], assumptions: [], ambiguity: [], conventions: [] },
        states: [s0],
        transformations: [],
        invariants: [],
        confidence: createConfidence(0.95),
