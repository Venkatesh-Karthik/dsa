...
describe("Cognora Intelligence 3.0: Universal Semantic Teaching Engine", () => {
  describe("Property 1: State transitions are deterministic", () => {
    it("produces identical successor states given the same operations", () => {
      const s0 = createSemanticState(0, "s0", {
        entities: new Map([
          ["node-10", createSemanticEntity("node-10", "TreeNode", "10", {}, { value: 10 })],
          ["node-20", createSemanticEntity("node-20", "TreeNode", "20", {}, { value: 20 })],
        ]),
      });
  ...
  describe("Property 2: Invariants are actually executed and never silently skipped", () => {
    it("actively detects and fails an invalid BST state", () => {
      const invalidBST = createSemanticState(0, "invalid-bst", {
        entities: new Map([
          ["root", createSemanticEntity("root", "TreeNode", "20", {}, { value: 20 })],
          ["left", createSemanticEntity("left", "TreeNode", "30", {}, { value: 30 })],
        ]),
        relationships: new Map([
          ["rel-1", createSemanticRelationship("rel-1", "root", "left", "left")],
        ]),
      });
    ...
    it("actively detects and fails an unbalanced AVL state", () => {
      const unbalancedTree = createSemanticState(0, "unbalanced", {
        entities: new Map([
          ["n30", createSemanticEntity("n30", "TreeNode", "30", {}, { value: 30 })],
          ["n20", createSemanticEntity("n20", "TreeNode", "20", {}, { value: 20 })],
          ["n10", createSemanticEntity("n10", "TreeNode", "10", {}, { value: 10 })],
        ]),
        relationships: new Map([
  ...
  describe("Property 5: Single Entity Identity across layers", () => {
    it("preserves stable IDs from SemanticEntity to SceneGraph and CompiledTimeline", () => {
      const result = UniversalConceptIntelligenceEngine.processQuestion("Explain TCP Connection", {
        entities: [
          createSemanticEntity("client-host", "Client", "Client"),
          createSemanticEntity("server-host", "Server", "Server"),
        ],
        relationships: [
          createSemanticRelationship("syn-edge", "client-host", "server-host", "connects"),
        ],
        steps: [
          {
            title: "Send SYN",
            explanation: "Client initiates handshake",
            visual_actions: [{ type: "highlight", target: "client-host" } as any],
          },
        ],
      });
  ...
  describe("Property 6: Native Excalidraw Arrow Bindings", () => {
    it("attaches startBinding and endBinding when reconciling connectors", () => {
      const graph = {
        entities: new Map([
          ["node-a", { id: "node-a", primitiveType: "GenericEntity", label: "A" }],
          ["node-b", { id: "node-b", primitiveType: "GenericEntity", label: "B" }],
        ]),
        relationships: new Map([
          ["edge-ab", { id: "edge-ab", sourceEntityId: "node-a", targetEntityId: "node-b", type: "connects" }],
        ]),
      };
      ...
      const reconciled = reconcileSceneState(sceneState as any, [], "test-lesson");

      const arrow = reconciled.elements.find((el) => el.type === "arrow") as any;
      expect(arrow).toBeDefined();
      expect(arrow.startBinding).toBeDefined();
      expect(arrow.endBinding).toBeDefined();
      expect(arrow.startBinding.elementId).toBe(reconciled.primaryElementMap.get("node-a")?.id);
      expect(arrow.endBinding.elementId).toBe(reconciled.primaryElementMap.get("node-b")?.id);
    });
  });
  ...
  describe("Domain Correctness Battery", () => {
          ...
          steps: [
            {
              ...
              explanation: "Node 20 rotates up to become root; 10 becomes its left child, 30 remains right child",
              operations: [
                { type: "disconnect_relation", id: "r1" },
                { type: "connect_relation", id: "r-new-left", source: "n20", target: "n10", relationType: "left" },
              ],
            },
          ],
        },
    ...
    it("2. Networking: HTTP GET Lifecycle", () => {
      const result = UniversalConceptIntelligenceEngine.processQuestion("Explain HTTP GET request lifecycle", {
        entities: [
        ...
        relationships: [
          createSemanticRelationship("http-req", "browser", "api-server", "http_get", "forward", { label: "GET /users" }),
          createSemanticRelationship("db-query", "api-server", "db", "sql_query", "forward", { label: "SELECT *" }),
        ],
        steps: [
          {
          ...
          },
          {
            ...
            operations: [
              { type: "connect_relation", id: "resp", source: "api-server", target: "browser", relationType: "http_200", label: "200 OK" },
            ],
          },
        ],
      });
    ...
    it("3. Operating Systems: Process Scheduling", () => {
      const result = UniversalConceptIntelligenceEngine.processQuestion("Simulate Round Robin CPU scheduling", {
        entities: [
        ...
        steps: [
          {
            ...
            operations: [
              ...
              { type: "connect_relation", id: "exec2", source: "p2", target: "cpu", relationType: "running" },
            ],
          },
        ],
      });

      expect(result.authoritativeModel.states[1].relationships.has("exec2")).toBe(true);
      expect(result.authoritativeModel.goalSatisfaction.satisfied).toBe(true);
    });

    it("4. Machine Learning: Gradient Descent Parameter Update", () => {
      const result = UniversalConceptIntelligenceEngine.processQuestion("Explain Gradient Descent parameter update", {
        entities: [
          createSemanticEntity("weight", "Parameter", "Weight W", { value: 2.5 }),
          createSemanticEntity("loss", "Metric", "MSE Loss", { value: 1.8 }),
          createSemanticEntity("grad", "Derivative", "dLoss/dW", { value: 0.4 }),
        ],
        relationships: [
          createSemanticRelationship("updates", "grad", "weight", "updates_with_lr"),
        ],
        steps: [
          {
            title: "Gradient Step",
      ...
      expect(result.authoritativeModel.states[1].entities.get("weight")?.value).toBe(2.46);
      expect(result.authoritativeModel.goalSatisfaction.satisfied).toBe(true);
    });
    ...
    it("5. Completely Unseen Concept: Raft Leader Election", () => {
      const result = UniversalConceptIntelligenceEngine.processQuestion(
        ...
        {
          entities: [
            createSemanticEntity("node1", "ServerNode", "Node 1 (Candidate)", { state: "candidate" }),
            createSemanticEntity("node2", "ServerNode", "Node 2 (Follower)", { state: "follower" }),
            createSemanticEntity("node3", "ServerNode", "Node 3 (Follower)", { state: "follower" }),
          ],
          relationships: [
            createSemanticRelationship("vote1", "node2", "node1", "vote_granted"),
          ],
          steps: [
            {
              title: "Quorum Achieved",
              explanation: "Node 1 receives vote from Node 2; 2 of 3 votes achieves majority quorum. Node 1 becomes Leader.",
              operations: [
                { type: "update_entity", entityId: "node1", state: "leader", label: "Node 1 (Leader)" },
                { type: "connect_relation", id: "heartbeat", source: "node1", target: "node3", relationType: "heartbeat" },
              ],
            },
          ],
        },
      ...
      const finalLeader = result.authoritativeModel.states[1].entities.get("node1");
    ...
    it("6. Completely Unseen Concept: Lac Operon Gene Regulation", () => {
      const result = UniversalConceptIntelligenceEngine.processQuestion(
        "Explain Lac Operon gene regulation switch in presence of lactose",
        {
          ...
          relationships: [
            createSemanticRelationship("blocks", "repressor", "operator", "binds_and_represses"),
          ],
          steps: [
            {
              title: "Inducer Binding",
              explanation: "Allolactose binds to repressor causing conformational change and detachment from operator",
              operations: [
                { type: "disconnect_relation", id: "blocks" },
                { type: "connect_relation", id: "allosteric", source: "allolactose", target: "repressor", relationType: "binds" },
                { type: "connect_relation", id: "transcribe", source: "rna_poly", target: "operator", relationType: "transcribes" },
              ],
            },
          ],
        },
      ...
      expect(result.authoritativeModel.states[1].relationships.has("blocks")).toBe(false);
      expect(result.authoritativeModel.states[1].relationships.has("transcribe")).toBe(true);
      expect(result.authoritativeModel.goalSatisfaction.satisfied).toBe(true);
    });
          ...
          steps: [
            {
              ...
              operations: [
                { type: "connect_relation", id: "p1", source: "prover", target: "verifier", relationType: "sends_commit" },
              ],
            },
            {
              ...
              operations: [
                { type: "connect_relation", id: "p2", source: "verifier", target: "prover", relationType: "sends_challenge" },
              ],
            },
            {
              title: "Pass 3: Response & Verification",
              explanation: "Prover computes s = r + e*x; Verifier verifies s*G == Commitment + e*PublicKey",
              operations: [
                { type: "connect_relation", id: "p3", source: "prover", target: "verifier", relationType: "sends_response" },
              ],
            },
          ],
        },
