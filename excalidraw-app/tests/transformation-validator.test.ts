...
describe("Semantic Fingerprinting & Diffing", () => {
  it("produces identical fingerprints for identical semantic content regardless of spatial layout", () => {
    ...
    const stateA = createSceneState(graphA, new Map([["box-1", { x: 100, y: 100 }]]));
    const stateB = createSceneState(graphB, new Map([["box-1", { x: 999, y: 999 }]]));
  ...
  it("produces different fingerprints when semantic properties, highlights, or values change", () => {
    ...
    expect(computeSemanticFingerprint(stateA)).not.toBe(computeSemanticFingerprint(stateB));
...
describe("Tree Depth & Complexity Validation", () => {
  ...
  it("rejects tree lessons with fewer than 2 levels unless triviality is explicitly requested", () => {
    ...
    const res1 = validateTreeComplexity(singleNodeState, "Explain AVL tree rotation");
    expect(res1.valid).toBe(false);
    expect(res1.error).toContain("minimum meaningful depth of at least 2 levels");

    // Explicit trivial request should accept single node
    const res2 = validateTreeComplexity(singleNodeState, "Show a single node tree example");
  ...
  it("verifies requested tree complexity (e.g. height 5)", () => {
    ...
    const check = validateTreeComplexity(state, "Explain AVL rotation using a height-5 tree");
...
describe("AVL Rotation Validation", () => {
  it("validates a correct right rotation where node 20 becomes root and parent-child pointers update", () => {
    ...
    const valResult = validateAVLRotation(timeline, "Explain an AVL right rotation");
...
describe("BFS Traversal Validation", () => {
  ...
  it("rejects BFS lessons where initial and final states are identical", () => {
    ...
    expect(bfsRes.error).toContain("initial state and final state are identical");
  });
});
    ...
    const redundantLesson: VisualLesson = {
      ...
      transformations: [
        ...
        },
        {
          ...
          operations: [{ type: "highlight", target: "arr-1", color: "success" }],
        },
      ],
    };
