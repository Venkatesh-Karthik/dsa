    ...
    it("renders on-canvas explanation card with badge, title, text, calculation, and insight", () => {
      const actions: VisualAction[] = [
        ...
        },
        {
          ...
          stepNumber: 1,
          totalSteps: 4,
          explanation: "We compute mid = (low + high) / 2 = 2. Value at mid is 30.",
      ...
      expect(texts.some((t) => t.includes("Step 1 of 4: Check Midpoint"))).toBe(true);
      expect(texts.some((t) => t.includes("mid = (low + high) / 2"))).toBe(true);
      expect(texts.some((t) => t.includes("mid = floor((0 + 4) / 2)"))).toBe(true);
      expect(texts.some((t) => t.includes("Search interval cut in half"))).toBe(true);
    });
  });
});
