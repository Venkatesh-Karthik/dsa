...
describe("SceneGraph & SceneState", () => {
  ...
  it("guarantees state immutability through deep cloning", () => {
    ...
    expect(state0.graph.entities.get("server")!.properties?.color).toBe("primary");

    // State1 reflects mutation
    expect(state1.graph.entities.get("server")!.label).toBe("API Server (Busy)");
    expect(state1.graph.entities.get("server")!.properties?.color).toBe("danger");
  });

  it("tracks relationships as first-class objects and cascades deletions", () => {
    const graph = createEmptySceneGraph();

    addEntity(graph, { id: "client", primitiveType: "GenericEntity", label: "Client" });
    addEntity(graph, { id: "server", primitiveType: "GenericEntity", label: "Server" });
  ...
  it("compiles legacy visual actions into a canonical SceneGraph with stable relationships", () => {
    ...
    expect(graph.relationships.has("edge-avl-tree-n30-avl-tree-n20")).toBe(true);
    expect(graph.relationships.has("edge-avl-tree-n20-avl-tree-n10")).toBe(true);
  });
});
...
describe("SceneDiff", () => {
  it("computes accurate diff between two SceneStates", () => {
    ...
    // Add n30
    addEntity(g1, { id: "n30", primitiveType: "TreeNode", value: 30, properties: { highlight: "success" } });
    // Connect n20 -> n30
    addRelationship(g1, {
      id: "e-2",
      type: "rightOf",
