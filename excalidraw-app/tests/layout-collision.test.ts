...
describe("Layout Engine - Text Measurement & Word Wrapping", () => {
  it("wraps long text across multiple lines without splitting words", () => {
    const text = "The subtree is left-heavy because the new node was inserted into the left subtree.";
...
describe("Layout Engine - Collision Detection & Resolution", () => {
  ...
  it("resolves collisions by shifting lower-priority items with clearance", () => {
    const boxes: CollisionBox[] = [
      { id: "node-1", x: 100, y: 100, width: 70, height: 70, fixed: true, priority: 10 },
      { id: "callout-1", x: 110, y: 110, width: 140, height: 50, fixed: false, priority: 1 },
    ];
    ...
    const finalBoxB = { x: calloutPos.x, y: calloutPos.y, width: 140, height: 50 };
