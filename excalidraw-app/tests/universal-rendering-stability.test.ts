...
import type { ExcalidrawElement, ExcalidrawArrowElement } from "@excalidraw/element/types";
...
describe("Universal Visual Rendering & Stability Engine", () => {
  ...
  it("1. creates a table with authoritative bounding box and deterministic scoped child IDs", () => {
    ...
    const cellEl = tableResult.allElements.find((e) => e.id === "students_table__cell_0_1");
  ...
  it("2. updates existing table in-place preserving sub-element identities", () => {
    ...
    const updatedElements = updateTablePrimitive(initial.allElements, entity, { x: 100, y: 100 });
  ...
  it("3. inserts row preserving existing row identities and adapting layout", () => {
    ...
    const updated = updateTablePrimitive(initial.allElements, entityWithNewRow, { x: 100, y: 100 });
  ...
  it("4. removes row cleanly, marks removed cells deleted, retains remaining rows", () => {
    ...
    const updated = updateTablePrimitive(initial.allElements, entityWithDeletedRow, { x: 100, y: 100 });
  ...
  it("5. dynamically measures column widths based on cell text length to prevent clipping", () => {
    ...
    const rows = [
      { key: "r1", values: ["x", "Extended explanatory text content that requires width"] },
    ];
  ...
  it("6. mutates single cell value in place without altering container geometry", () => {
    ...
    const updated = updateTablePrimitive(initial.allElements, entity, { x: 100, y: 100 });
  ...
  it("7. highlights row without altering layout or shifting cell coordinates", () => {
    ...
    const entity: SemanticEntity = {
      ...
      properties: {
        ...
        rows: [
          ["Alice", 95],
          { values: ["Bob", 80], highlight: "success" },
        ],
        highlightRowIndex: 1,
      },
    };
  ...
  it("9. connects two tables cleanly using authoritative bounding boxes", () => {
    ...
    const arrow = reconciled.elements.find((e) => e.type === "arrow") as ExcalidrawArrowElement;
    expect(arrow).toBeDefined();
    expect(arrow.customData?.dslId).toBe("rel_fk");

    const edgeLabel = reconciled.elements.find((e) => e.customData?.dslId === "rel_fk-label");
  ...
  it("10. positions source tables side by side and places result table centered below", () => {
    ...
    const graph: SceneGraph = {
      ...
      relationships: new Map([
        ["rel1", { id: "rel1", sourceEntityId: "users", targetEntityId: "result_table", type: "derivedFrom" }],
        ["rel2", { id: "rel2", sourceEntityId: "orders", targetEntityId: "result_table", type: "derivedFrom" }],
      ]),
      annotations: new Map(),
    };
  ...
  it("11. translates all child elements of a table by identical (dx, dy) with zero distortion", () => {
    ...
    const dx = 150;
    const dy = 80;
    const moved = updateTablePrimitive(initial.allElements, entity, { x: 100 + dx, y: 100 + dy });
  ...
  it("13. rejects dangling connectors when endpoints are deleted or invalid", () => {
    ...
    const reconciled = reconcileSceneState(sceneState, []);
    const arrows = reconciled.elements.filter((e) => e.type === "arrow" && !e.isDeleted);
  ...
  it("14. preserves annotation element ID across reconciliation steps", () => {
    const graph1: SceneGraph = {
      entities: new Map([
        ["node1", { id: "node1", primitiveType: "TreeNode", semanticRole: "state", value: 10 }],
      ]),
      relationships: new Map(),
      annotations: new Map([
        ["ann_balance", { id: "ann_balance", type: "badge", text: "Balance factor: +1", targetEntityId: "node1" }],
      ]),
    };

    const res1 = reconcileSceneState({ graph: graph1, layoutState: new Map([["node1", { x: 100, y: 100 }]]) }, []);
    const ann1 = res1.elements.find((e) => e.customData?.dslId === "ann_balance");
    ...
    const graph2: SceneGraph = {
      ...
      relationships: new Map(),
      annotations: new Map([
        ["ann_balance", { id: "ann_balance", type: "badge", text: "Balance factor: 0 (Balanced)", targetEntityId: "node1" }],
      ]),
    };

    const res2 = reconcileSceneState({ graph: graph2, layoutState: new Map([["node1", { x: 100, y: 100 }]]) }, res1.elements);
    const ann2 = res2.elements.find((e) => e.customData?.dslId === "ann_balance");
  ...
  it("15. produces deterministic layout positions for identical semantic scene graphs", () => {
    const graph: SceneGraph = {
      entities: new Map([
        ["e1", { id: "e1", primitiveType: "TreeNode", semanticRole: "state", value: 50 }],
        ["e2", { id: "e2", primitiveType: "TreeNode", semanticRole: "state", value: 30 }],
      ]),
      relationships: new Map([
        ["r1", { id: "r1", sourceEntityId: "e1", targetEntityId: "e2", type: "leftOf" }],
      ]),
      annotations: new Map(),
    };
  ...
  it("24. ensures ArrayCell and StackFrame compound components preserve identity and derive geometry", () => {
    ...
    const updated = updateVisualPrimitive(primitive.allElements, cellEntity, { x: 150, y: 80 });
  ...
  it("25. demonstrates universal rendering across completely unrelated domains", () => {
    const domains = [
      {
        ...
        entities: [
          { id: "t1", primitiveType: "Table" as const, semanticRole: "state" as const, properties: { columns: ["id", "val"], rows: [["1", "A"]] } },
          { id: "t2", primitiveType: "Table" as const, semanticRole: "output" as const, properties: { columns: ["id", "val"], rows: [["1", "A"]] } },
        ],
      ...
      },
      {
        ...
        entities: [
          { id: "fetch", primitiveType: "ProcessNode" as const, semanticRole: "process" as const, label: "Fetch" },
          { id: "decode", primitiveType: "ProcessNode" as const, semanticRole: "process" as const, label: "Decode" },
          { id: "execute", primitiveType: "ProcessNode" as const, semanticRole: "process" as const, label: "Execute" },
        ],
      ...
      },
      {
        ...
        entities: [
          { id: "client", primitiveType: "Client" as const, semanticRole: "actor" as const, label: "Client" },
          { id: "server", primitiveType: "Server" as const, semanticRole: "actor" as const, label: "Server" },
        ],
      ...
      },
      {
        ...
        entities: [
          { id: "root", primitiveType: "TreeNode" as const, semanticRole: "state" as const, value: 30, properties: { left: "left_child" } },
          { id: "left_child", primitiveType: "TreeNode" as const, semanticRole: "state" as const, value: 20 },
        ],
      ...
      },
      {
        ...
        entities: [
          { id: "air", primitiveType: "Medium" as const, semanticRole: "context" as const, label: "Air (n=1.0)" },
          { id: "glass", primitiveType: "Medium" as const, semanticRole: "context" as const, label: "Glass (n=1.5)" },
          { id: "beam", primitiveType: "Ray" as const, semanticRole: "energy" as const, label: "Incident Ray" },
        ],
      ...
      },
      {
        ...
        entities: [
          { id: "chloroplast", primitiveType: "Container" as const, semanticRole: "container" as const, label: "Chloroplast" },
          { id: "light_rxn", primitiveType: "ProcessNode" as const, semanticRole: "process" as const, label: "Light Reactions" },
        ],
        strategy: "flow" as const,
      },
    ];
    ...
    for (const domain of domains) {
      ...
      const reconciled = reconcileSceneState(sceneState, []);
      expect(reconciled.elements.length).toBeGreaterThanOrEqual(domain.entities.length);
