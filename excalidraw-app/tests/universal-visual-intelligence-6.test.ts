...
describe("Universal Visual Intelligence 6.0 Platform", () => {
  ...
  describe("3. Dynamic Visual Requirement Extraction", () => {
    it("derives required entities, relationships, states, focus, and cognitive load", () => {
      const mockModel: AuthoritativeSemanticModel = {
        ...
        world: {
          entities: [
            { id: "client", label: "Client", type: "Client", semanticRole: "client" },
            { id: "server", label: "Server", type: "Server", semanticRole: "server" },
          ],
          relationships: [
            { id: "r1", sourceEntityId: "client", targetEntityId: "server", type: "sends", label: "SYN" },
          ],
          states: [],
        } as any,
        rules: [],
        ...
        states: [
          {
            name: "Initial",
            entities: [
              { id: "client", label: "Client", type: "Client", state: "LISTEN" },
              { id: "server", label: "Server", type: "Server", state: "LISTEN" },
            ],
            relationships: [],
          },
        ] as any,
        ...
        goalSatisfaction: { satisfied: true, objective: "Handshake", verifiedCriteria: [], summary: "OK" },
        strategy: "visual",
  ...
  describe("4. Scene Composition Engine", () => {
    it("preserves stable semantic identity across multiple transformations", () => {
      const mockReqs: VisualRequirements = {
        ...
        layoutStrategy: "dag",
        requiredEntities: [
          { id: "account-a", label: "Account A", visualType: "Record", semanticRole: "source", importance: "primary", persistent: true, value: 500 },
          { id: "account-b", label: "Account B", visualType: "Record", semanticRole: "target", importance: "primary", persistent: true, value: 200 },
        ],
        requiredRelationships: [
          { id: "rel-transfer", source: "account-a", target: "account-b", type: "transfers", direction: "forward", importance: "primary", label: "$100" },
        ],
        requiredStates: [
          { stateIndex: 0, name: "Initial", stateType: "normal", invariantBadges: ["Total Balance = $700"] },
        ],
        causalFlows: [],
        evidenceRequirements: [],
        focus: { primaryFocusId: "account-a", secondaryFocusIds: ["account-b"], backgroundEntityIds: [] },
        cognitiveLoad: { density: "low", maxPrimaryItems: 8, omittedEntityIds: [] },
      };
      ...
      const scene1 = composeVisualScene(mockReqs, scene0, { stepIndex: 1, activeDiffSummary: "Transferred $100" });
  ...
  describe("5. Universal Spatial Layout & Collision Engine", () => {
    it("computes collision-free layout for dense multi-node graphs", () => {
      ...
      graph.relationships.set("r1-2", { id: "r1-2", sourceEntityId: "node-1", targetEntityId: "node-2", type: "flows" });
      graph.relationships.set("r2-3", { id: "r2-3", sourceEntityId: "node-2", targetEntityId: "node-3", type: "flows" });
      graph.relationships.set("r3-4", { id: "r3-4", sourceEntityId: "node-3", targetEntityId: "node-4", type: "flows" });
      graph.relationships.set("r4-5", { id: "r4-5", sourceEntityId: "node-4", targetEntityId: "node-5", type: "flows" });
      graph.relationships.set("r5-6", { id: "r5-6", sourceEntityId: "node-5", targetEntityId: "node-6", type: "flows" });
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          expect(collides, `Collision detected between ${nodes[i].id} and ${nodes[j].id}`).toBe(false);
        }
      }
    });
    ...
    it("positions adaptive annotations without occluding primary entities", () => {
      ...
      const bestPos = findBestAnnotationPosition(anchorBounds, annBounds, obstacles, "above", 20);
    ...
    it("guarantees text measurement and wrapping never produce unreadable clipping", () => {
      const longText = "This is a comprehensive educational explanation that must wrap gracefully without clipping.";
  ...
  describe("6. Semantic Animation Planning", () => {
    it("derives purposeful animation actions from state graph diffs", () => {
      const prevGraph: SceneGraph = {
        entities: new Map([
          ["client", { id: "client", primitiveType: "Client", state: "LISTEN", label: "Client" }],
          ["server", { id: "server", primitiveType: "Server", state: "LISTEN", label: "Server" }],
        ]),
        relationships: new Map(),
        annotations: new Map(),
      };
      ...
      const nextGraph: SceneGraph = {
        entities: new Map([
          ["client", { id: "client", primitiveType: "Client", state: "SYN_SENT", label: "Client" }],
          ["server", { id: "server", primitiveType: "Server", state: "LISTEN", label: "Server" }],
          ["syn-pkt", { id: "syn-pkt", primitiveType: "Message", label: "SYN [seq=100]" }],
        ]),
        relationships: new Map([
          ["syn-rel", { id: "syn-rel", sourceEntityId: "client", targetEntityId: "server", type: "sends" }],
        ]),
        annotations: new Map(),
      };
      ...
      const stateChangeAct = plan.actions.find((a) => a.motionType === "STATE_CHANGE");
  ...
  describe("7. Visual Evidence Validator", () => {
    it("validates explanation claims against authoritative state and scene graph", () => {
      const mockModel: AuthoritativeSemanticModel = {
        id: "ev-model",
        problem: { question: "TCP 3-way handshake", intent: "EXPLAIN", objective: "Handshake" } as any,
        world: {
          entities: [
          ...
          relationships: [
            { id: "rel1", source: "client", target: "server", type: "sends", label: "SYN" },
          ],
          states: [],
        } as any,
        rules: [],
        invariants: [
          {
            ...
            predicate: (state: any) => (state.entities?.size || state.entities?.length || 0) >= 2,
          } as any,
        ],
        states: [
          {
            name: "SYN Sent",
            entities: new Map([
              ["client", { id: "client", label: "Client", type: "Client", state: "syn-sent" }],
              ["server", { id: "server", label: "Server", type: "Server", state: "listen" }],
            ]),
            relationships: new Map(),
          },
        ] as any,
        transformations: [
          {
            ...
            explanation: "The client transitions to syn-sent and sends SYN to server",
      ...
      const sceneGraph: SceneGraph = {
        entities: new Map([
          ["client", { id: "client", primitiveType: "Client", label: "Client", state: "syn-sent" }],
          ["server", { id: "server", primitiveType: "Server", label: "Server", state: "listen" }],
        ]),
        relationships: new Map([
          ["rel1", { id: "rel1", sourceEntityId: "client", targetEntityId: "server", type: "sends", label: "SYN" }],
        ]),
        annotations: new Map(),
      };
    ...
    it("detects unsupported claims if explanatory text mentions missing visual entity", () => {
      const mockModel: AuthoritativeSemanticModel = {
        id: "ev-model-2",
        problem: { question: "Proxy flow", intent: "EXPLAIN", objective: "Proxy" } as any,
        world: {
          entities: [
            { id: "client", label: "Client", type: "Client" },
        ...
        derivedValuesByState: {},
        goalSatisfaction: { satisfied: true, objective: "Proxy", verifiedCriteria: [], summary: "OK" },
        strategy: "visual",
        confidence: createConfidence(1, "KNOWN", "High"),
        timestamp: Date.now(),
      };
      ...
      const brokenSceneGraph: SceneGraph = {
        entities: new Map([
          ["client", { id: "client", primitiveType: "Client", label: "Client" }],
        ]),
        relationships: new Map(),
        annotations: new Map(),
      };
  ...
  describe("8. Cross-Domain Verification Matrix", () => {
    const domainPrompts = [
      ...
      { cat: "B. Complex Multi-Stage", prompt: "Explain two-phase commit protocol across distributed databases" },
      { cat: "C. Graph Concept", prompt: "Trace Dijkstra's shortest path algorithm on a weighted graph" },
      { cat: "D. Tree Concept", prompt: "Explain AVL tree insertion causing right rotation to restore balance" },
      { cat: "E. Array Concept", prompt: "Demonstrate in-place two-pointer quicksort partition" },
      { cat: "F. Sequence Timeline", prompt: "Trace asynchronous event loop message queue execution" },
      { cat: "G. State Machine", prompt: "Explain TCP connection states from LISTEN to ESTABLISHED and CLOSED" },
      { cat: "H. Database", prompt: "Explain database ACID transaction rollback upon constraint violation" },
      { cat: "I. Networking", prompt: "Explain DNS resolution from stub resolver to root and authoritative nameservers" },
      { cat: "J. Physics", prompt: "Explain light ray refraction across air and glass boundary according to Snell's law" },
      { cat: "K. Biology", prompt: "Explain light-dependent reaction and ATP synthesis in chloroplast thylakoid" },
      { cat: "L. Mathematics", prompt: "Explain gradient descent optimization taking steps towards the global minimum" },
      { cat: "M. Operating Systems", prompt: "Explain virtual memory page fault handling and swapping from disk to RAM" },
      { cat: "N. Distributed Systems", prompt: "Explain Raft leader election heartbeat and term increment" },
      { cat: "O. Unknown Concept Fallback", prompt: "Explain chronon particle oscillation across tachyon boundary field" },
      { cat: "P. Failure and Recovery", prompt: "Show transaction failure during debit and rollback recovery" },
      { cat: "Q. Decision Branches", prompt: "Show binary search branching left when key is smaller than midpoint" },
      { cat: "R. Counterfactual Reasoning", prompt: "What happens if the acknowledgment packet is lost in transit?" },
    ];
    ...
    for (const testCase of domainPrompts) {
      it(`successfully generates and compiles universal visual lesson for: ${testCase.cat} (${testCase.prompt})`, async () => {
        const result = await UniversalConceptIntelligenceEngine.teach(testCase.prompt);
        ...
        expect(initialState.layoutState?.size).toBe(initialState.graph.entities.size);
        ...
        expect(step0Meta.evidenceValidation?.confidenceScore).toBeGreaterThanOrEqual(0.7);
      });
    }
  ...
  describe("9. Anti-Hardcoding Architectural Audit", () => {
    it("verifies the Visual Capability Registry is topic-agnostic", () => {
      ...
      for (const cap of caps) {
        ...
        expect(cap.id).not.toMatch(/^(?:AVL|TCP|DNS|HTTP|Refraction|Photosynthesis)/);
      }
    });
    ...
    it("verifies compound components are architectural compositions, not topic renderers", () => {
      ...
      for (const comp of compounds) {
        expect(comp.id).not.toMatch(/^(?:AVLRenderer|TCPRenderer|PhotosynthesisRenderer|RefractionRenderer)/);
      }
    });
  });
});
