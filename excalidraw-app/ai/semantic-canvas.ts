...
export function createSemanticSnapshot(
  ...
  for (const el of elements) {
    ...
    const value = el.customData?.value !== undefined
    ...
    snapshot.set(dslId, {
      ...
      role,
      structureId,
      index: typeof el.customData?.index === "number" ? el.customData.index : undefined,
      x: Math.round(el.x),
      y: Math.round(el.y),
    });
  }
  ...
  for (const [dslId, prev] of previousSnapshot.entries()) {
    ...
    if (!current) {
      deltas.push({
        ...
        oldValue: prev.value,
        description: `Element '${dslId}'${prev.role ? ` (${prev.role})` : ""} was removed from the canvas.`,
      });
      continue;
    }

    // 2. Value edited (e.g. 20 -> 9)
    const prevVal = prev.value !== undefined ? String(prev.value).trim() : undefined;
    const currVal = current.value !== undefined ? String(current.value).trim() : undefined;
...
export function extractSemanticCanvasState(
    ...
    if (dslId) {
      ...
      const textVal =
        el.type === "text"
          ...
          : containerTextMap.get(el.id) ?? (el.customData?.value as string | number | undefined);

      // 1. Array Element
      if (role === "array-element" || el.customData?.arrayId) {
        const arrId = (el.customData?.arrayId as string) || dslId.split("-")[0];
        const idx = typeof el.customData?.index === "number"
          ? el.customData.index
      ...
      // 2. Tree Node
        ...
        const nodeId = (el.customData?.nodeId as string) || dslId.replace(`${treeId}-`, "");
        const val = textVal ?? (el.customData?.value as string | number) ?? nodeId;
      ...
      // 3. Graph Node & Edge
        ...
        const nodeId = (el.customData?.nodeId as string) || dslId.replace(`${gId}-`, "");
      ...
      // 5. Pointer Annotation
        ...
        pointers.push({
          id: dslId,
          label: (el.customData?.label as string) || (textVal !== undefined ? String(textVal) : "") || "POINTER",
  ...
  if (arraysMap.size > 0) {
    for (const arr of arraysMap.values()) {
      ...
      summaryLines.push(`• Array '${arr.id}': [${vals}] (length: ${arr.elements.length})`);
      if (arr.pointers.length > 0) {
        const ptrStrs = arr.pointers.map((p) => `${p.label} -> target '${p.targetDslId}'`);
  ...
  if (treesMap.size > 0) {
    for (const tree of treesMap.values()) {
      ...
      for (const node of tree.nodes.slice(0, 8)) {
        ...
        summaryLines.push(`  Node '${node.nodeId}' (val: ${node.value})${children ? ` [${children}]` : ""}`);
      }
    }
  }
  ...
  if (graphsMap.size > 0) {
    for (const g of graphsMap.values()) {
      ...
      summaryLines.push(`• Graph '${g.id}' (${g.nodes.length} nodes: [${nodeLabels}], ${g.edges.length} edges)`);
    }
  }
