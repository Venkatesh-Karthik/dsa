...
export function composeVisualScene(
  ...
  for (const req of requirements.requiredEntities) {
    ...
    const cap = visualCapabilities.findMatching(req.visualType || req.semanticRole || req.label);
    ...
    if (!highlight && req.importance === "primary" && req.id === requirements.focus.primaryFocusId) {
      highlight = "active";
    }
    ...
    const entity: SemanticEntity = {
      ...
      properties: {
        ...
        highlight,
        shape: (req.properties?.shape as any) || (cap.id === "TreeNode" ? "ellipse" : "rectangle"),
        layer: req.importance === "primary" ? 1 : req.importance === "secondary" ? 2 : 3,
      },
    };
  ...
  for (const relReq of requirements.requiredRelationships) {
    ...
    if (!graph.entities.has(relReq.source) || !graph.entities.has(relReq.target)) {
      continue;
    }
  ...
  // Invariant Badge
  if (currStateReq?.invariantBadges && currStateReq.invariantBadges.length > 0) {
