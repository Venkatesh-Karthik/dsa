...
export interface VerifiedClaim {
  claimText: string;
  claimType: "entity_presence" | "state_change" | "relationship_flow" | "invariant_preservation" | "action";
  ...
  public static validateStepEvidence(
    ...
    for (const entity of worldEntities) {
      const isMentioned =
        ...
        (entity.label && explanationText.toLowerCase().includes(entity.label.toLowerCase()));

      const existsInAuthoritativeState =
        currentState.entities.has(entity.id) ||
        Array.from(currentState.entities.values()).some((e) => e.id === entity.id);
      ...
      if (isMentioned || existsInAuthoritativeState) {
        ...
        if (!existsInVisualScene) {
          claim.missingEvidenceReason = `Entity '${entity.label || entity.id}' (${entity.id}) is authoritative but missing from visual scene graph.`;
    ...
    const worldRelationships =
      model.world?.relationships || Array.from(currentState.relationships.values());
    for (const rel of worldRelationships) {
      ...
      const isMentioned =
        ...
        (rel.label && explanationText.toLowerCase().includes(rel.label.toLowerCase()));

      const existsInWorld = true;
      const existsInVisualScene = Array.from(sceneGraph.relationships.values()).some(
        (r) =>
          (r.sourceEntityId === rel.source && r.targetEntityId === rel.target) ||
          (r.sourceEntityId === rel.target && r.targetEntityId === rel.source && r.properties?.directed === false),
      );
      ...
      for (const stateWord of stateMentions) {
        ...
        for (const [id, visualEnt] of sceneGraph.entities.entries()) {
          ...
          if (
            ...
            matchingIds.push(id);
            if (visualEntityState.includes(stateWord) || visualLabel.includes(stateWord)) {
    ...
    for (const inv of activeInvariants) {
      ...
      try {
        ...
        } else if (inv.category === "structural" || inv.statement.toLowerCase().includes("unique")) {
          const ids = Array.from(currentState.entities.keys());
          passed = new Set(ids).size === ids.length;
          if (!passed) reason = "Entity IDs are not unique in state.";
      ...
      if (!passed) {
        const failureMsg = `Invariant '${invName}' violated in step ${stepIndex}: ${reason || inv.description || inv.statement}`;
    ...
    const verifiedCount = verifiedClaims.filter(
      (c) => c.semanticFactFound && c.authoritativeStateVerified && c.visualEvidenceFound,
    ).length;
