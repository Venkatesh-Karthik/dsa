...
import type { CreateArrayAction, ArrayElementHighlight, SemanticColor } from "../visual-dsl";
...
export function renderArrayGrammar(
  ...
  const originY = action.y !== undefined ? action.y : context.cursor.y + titleHeight;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const strokeColor = mappedColor ? mapStrokeColor(mappedColor) : mapStrokeColor(undefined);
    const backgroundColor = mappedColor ? mapBackgroundColor(mappedColor) : undefined;
