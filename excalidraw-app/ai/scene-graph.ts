 ...
 */
export function createEmptySceneGraph(metadata?: SceneGraph["metadata"]): SceneGraph {
  return {
    entities: new Map(),
...
export function cloneSceneGraph(graph: SceneGraph): SceneGraph {
  const cloned: SceneGraph = {
    ...
    relationships: new Map(),
    annotations: new Map(),
    metadata: graph.metadata ? JSON.parse(JSON.stringify(graph.metadata)) : undefined,
  };

  for (const [id, entity] of graph.entities.entries()) {
    cloned.entities.set(id, {
      ...entity,
      properties: entity.properties ? JSON.parse(JSON.stringify(entity.properties)) : undefined,
    });
  }
  ...
  for (const [id, rel] of graph.relationships.entries()) {
    cloned.relationships.set(id, {
      ...rel,
      properties: rel.properties ? JSON.parse(JSON.stringify(rel.properties)) : undefined,
    });
  }
  ...
  for (const [id, ann] of graph.annotations.entries()) {
    cloned.annotations.set(id, {
      ...ann,
      properties: ann.properties ? JSON.parse(JSON.stringify(ann.properties)) : undefined,
    });
  }
...
export function removeEntity(graph: SceneGraph, id: SemanticEntityId): boolean {
  if (!graph.entities.has(id)) return false;
 ...
 */
export function addRelationship(graph: SceneGraph, rel: SemanticRelationship): void {
