...
export function createGenericEntity(props: GenericEntityProps): GenericEntityPrimitive {
  const {
    id,
    x,
    y,
  ...
  const entity = newElement({
    ...
    roughness: 0,
    roundness: shape === "rectangle" ? { type: ROUNDNESS.ADAPTIVE_RADIUS } : null,
    groupIds: [groupId],
    customData: {
      dslId: id,
      role,
  ...
  const finalEntity = shape === "rectangle" 
    ? newElementWith(entity, { boundElements: [{ type: "text", id: textLabel.id }] })
    : entity;
