...
export interface TableRow {
  ...
  highlight?: "active" | "matched" | "filtered" | "danger" | "success" | "warning" | "default" | string;
}
...
function normalizeColumns(
  ...
  if (rawColumns && rawColumns.length > 0) {
    return rawColumns.map((col, idx) => {
      ...
      return { key: col.key || `col_${idx}`, title: col.title || col.key || `Col ${idx + 1}`, width: col.width };
    });
  }
  ...
  if (rows && rows.length > 0) {
    ...
    if ("values" in first && Array.isArray(first.values)) {
      return first.values.map((_, i) => ({ key: `col_${i}`, title: `Col ${i + 1}` }));
    }
  }
...
function normalizeRows(
  ...
  return rawRows.map((row, rIdx) => {
    ...
    if ("values" in row && Array.isArray(row.values)) {
      return { key: row.key || `row_${rIdx}`, values: row.values, highlight: row.highlight };
    }
    if (typeof row === "object" && row !== null && columns) {
      const vals = columns.map((c) => (row as Record<string, any>)[c.key] ?? "");
      return { key: `row_${rIdx}`, values: vals, highlight: (row as any).highlight };
    }
...
export function computeTableColumnWidths(
  ...
  for (let c = 0; c < columns.length; c++) {
    ...
    colWidths.push(Math.max(minColWidth, Math.min(260, Math.round(maxW + padding))));
  }
 ...
 */
export function createTablePrimitive(props: TablePrimitiveProps): TablePrimitiveResult {
  const {
    id,
    x,
    y,
  ...
  const rowHeight = 30;
  const totalTableHeight = titleHeight + headerHeight + Math.max(1, rows.length) * rowHeight;
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    const isRowDanger = row.highlight === "danger" || row.highlight === "eliminated";
...
export function updateTablePrimitive(
  ...
  if ("primitiveType" in propsOrEntity) {
    ...
    props = {
      ...
      highlightRowIndex: entity.properties?.highlightRowIndex as number | undefined,
