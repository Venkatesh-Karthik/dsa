...
export function validateVisualAction(
  ...
  if (isCreation) {
    if (typeof act.id !== "string" || act.id.trim().length === 0) {
      errors.push(`${prefix}.id must be a non-empty string for "${act.type}".`);
    } else {
      if (seenIds.has(act.id)) {
        errors.push(`${prefix}.id "${act.id}" is duplicated in this batch.`);
      }
      seenIds.add(act.id);
    }
  }
  ...
  if (isManipulation) {
    if (typeof act.target !== "string" || act.target.trim().length === 0) {
      errors.push(
        `${prefix}.target must be a non-empty string for "${act.type}".`,
      );
    }
  }
