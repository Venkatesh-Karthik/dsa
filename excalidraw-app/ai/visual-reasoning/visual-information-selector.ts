...
import { inferEntityHierarchy, type VisualHierarchyLevel } from "./visual-hierarchy";
...
export class VisualInformationSelector {
  ...
  public static select(input: {
    ...
    for (const ent of input.entities) {
      ...
      const rawText = ent.label || (typeof ent.value === "string" ? ent.value : ent.id);
    ...
    for (const rel of input.relationships) {
      ...
      const rawLabel = rel.label || (rel.properties?.label as string | undefined);
      ...
      const hierarchy: VisualHierarchyLevel = isPrimaryRel ? "SECONDARY" : "TERTIARY";
      const maxLabelLen = density > 0.5 ? 20 : 28;
      const compressedLabel = sanitizedLabel ? compressSemanticPayload(sanitizedLabel, maxLabelLen) : undefined;
