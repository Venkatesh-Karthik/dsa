...
export class VisualCompositionPlanner {
  ...
  public static plan(
    ...
    const entityRoles = new Set(entities.map((e) => (e.semanticRole || "").toLowerCase()));
    const relTypes = new Set(relationships.map((r) => (r.type || "").toLowerCase()));

    // 1. Two-party / Multi-actor communication check
    const hasActors =
      entities.some((e) => {
        const s = `${e.label || ""} ${e.type || ""} ${e.semanticRole || ""}`.toLowerCase();
        return (
          s.includes("client") ||
          s.includes("sender") ||
    ...
    const hasTables =
      ...
      entities.some((e) => {
        ...
        return s.includes("account") || s.includes("table") || s.includes("ledger");
      });
    ...
    const hasDecisions =
      ...
      states.some((s) => s.stateType === "decision" || s.stateType === "failure") ||
      transformations.some((t) => t.decision || t.branchType === "failure" || t.stateType === "failure");

    // 8. Mathematical / spatial physics check
    const hasMathOrSpatial =
      entities.some((e) => {
    ...
    if (hasActors && (hasMessages || relationships.length > 0)) {
      ...
      if (hasDecisions) secondaryStrategies.push("state-transition");
