...
export class VisualReasoningEngine {
  ...
  public static compileSceneGraphForState(
    ...
    const trans = stateIndex > 0 ? model.transformations[stateIndex - 1] : undefined;
    ...
    for (const [id, semEnt] of semState.entities.entries()) {
      ...
      } else if (semEnt.state === "failed" || semState.stateType === "failure") {
    ...
    for (const [id, semRel] of semState.relationships.entries()) {
      ...
      if (!plannedRel.isConnectorNeeded) continue;

      // Only add if both endpoints are in the current state's graph
      if (!graph.entities.has(plannedRel.source) || !graph.entities.has(plannedRel.target)) {
        continue;
      }

      let relHighlight: string | undefined = semRel.properties?.highlight as string | undefined;
