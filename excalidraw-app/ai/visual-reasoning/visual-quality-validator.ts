  ...
  public static validate(
    ...
    for (const ent of entities) {
      if (!elementPlan.elements.has(ent.id)) {
        ...
        if (ev?.shouldRender) {
          ...
          defects.push({
            code: "MISSING_ENTITY",
            message: `Authoritative entity '${ent.label || ent.id}' (${ent.id}) is missing from visual elements.`,
            severity: "critical",
            entityId: ent.id,
    ...
    if (allGenericBoxes) {
      ...
      defects.push({
        ...
        message: "All entities defaulted to generic rectangle boxes without semantic specialization.",
        severity: "critical",
        remedyHint: "Specialize visual representations based on semantic roles.",
      });
    }
    ...
    for (const rel of relationships) {
      ...
      if (!planned) {
        ...
        if (planned.direction === "none" && rel.direction !== "none") {
          directionClarity -= 10;
          defects.push({
    ...
    if (!spatialPlan.focalEntityId) {
      ...
      defects.push({
        code: "NO_FOCAL_POINT",
        message: "Spatial plan lacks a clear focal starting point for learner reading order.",
        severity: "warning",
      });
    }
    ...
    let annotationSafety = 100;

    if (spatialPlan.minSpacingX < 20 || spatialPlan.minSpacingY < 20) {
      collisionSafety -= 25;
      defects.push({
        code: "INSUFFICIENT_SPACING",
        message: "Spatial spacing buffers are below safe anti-collision threshold.",
        severity: "critical",
    ...
    for (let sIdx = 1; sIdx < transformationPlan.steps.length; sIdx++) {
      ...
      if (deltaCount === 0) {
        ...
        defects.push({
          ...
          severity: "critical",
          stepIndex: sIdx,
          remedyHint: "Ensure step mutates state, highlights active entity, or updates relationships.",
        });
      }
    }
    ...
    let visualHierarchy = primaryCount > 0 ? 100 : 60;

    // 12. Information Density & 13. Redundancy Control
    let informationDensity = 100;
    ...
    // 14. Goal Visibility
    let goalVisibility = model.goalSatisfaction?.satisfied !== false ? 100 : 70;
