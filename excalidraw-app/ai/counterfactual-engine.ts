...
export interface CounterfactualResult {
  ...
  invariantViolations: Array<{ statement: string; severity: string; details?: string }>;
  consequences: string[];
  isViable: boolean;
...
export class CounterfactualEngine {
  ...
  public static evaluateWhatIf(
    ...
    const type = mutation.mutationType || (mutation.targetRelationshipId ? "relationship_break" : "property_change");
    ...
    switch (type) {
      case "entity_removal": {
        ...
        if (!ent) {
          consequences.push(`Target entity '${entId}' not found in current state.`);
          return {
      ...
      case "relationship_break": {
        ...
        if (rel) {
          ...
          consequences.push(`Relationship '${rel.label || rel.type}' between '${rel.source}' and '${rel.target}' was severed.`);
          causalChain.push(
          ...
          for (const [rid, r] of simulated.relationships.entries()) {
            if (r.source === mutation.targetEntityId) {
              simulated.relationships.delete(rid);
              consequences.push(`Disconnected outgoing relationship from '${r.source}'.`);
              found = true;
              break;
            }
          }
      ...
      case "step_failure": {
        ...
        consequences.push(
          `System entered failure state at step ${mutation.stepIndex ?? baseState.index}.`,
      ...
      case "condition_flip": {
        ...
        if (dec) {
          const altOutcome = dec.possibleOutcomes.find((o) => o.id !== dec.selectedOutcomeId) || dec.possibleOutcomes[1];
          if (altOutcome) {
            ...
            causalChain.push(
              ...
              `Follows alternative branch: ${altOutcome.consequences.join(", ") || "branch switch"}`,
            );
          }
        } else {
          consequences.push(`Alternative outcome evaluated. System branched away from primary path.`);
        }
        break;
      }
      ...
      default: {
        ...
        if (ent) {
          ...
          consequences.push(
            `Entity '${ent.label}' changed ${propKey} from ${String(oldValue)} to ${String(mutation.hypotheticalValue)}`,
          );
          causalChain.push(
    ...
    const report = InvariantEngine.validateStateInvariants(simulated, model.invariants, model.states);
