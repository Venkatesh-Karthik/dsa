...
export function renderVerticalLesson(
  ...
  for (let stepIdx = 0; stepIdx <= maxStepIdx; stepIdx++) {
    ...
    actionsToRender.push({
      ...
      text: `STEP ${stepIdx + 1} OF ${steps.length}: ${step.title.toUpperCase()}`,
...
export function appendLessonStep(
  ...
  actionsToRender.push({
    ...
    text: `STEP ${stepIndexToAppend + 1} OF ${steps.length}: ${step.title.toUpperCase()}`,
    style: { color: "primary", size: "md" },
  });
  ...
  if (lastRegion) {
    ...
    const dividerY =
      lastRegion.bounds.y +
      lastRegion.bounds.height +
      STEP_LAYOUT.STEP_GAP / 2;
    const dividerElements = renderActions(
      [
        {
          type: "create_divider",
