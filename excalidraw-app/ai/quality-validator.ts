...
export function validateAuthoritativeTeachingQuality(
  ...
  if (!teachingGoalExists) issues.push("Teaching goal is missing or empty.");

  const semanticEntitiesExist = model.world.entities.length > 0;
  if (!semanticEntitiesExist) issues.push("Semantic entities are missing.");
  ...
  if (!statesValid) issues.push("Semantic states sequence is empty.");
