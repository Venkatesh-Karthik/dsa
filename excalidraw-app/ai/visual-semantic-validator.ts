  ...
  public static validateVisualConsistency(
    ...
    for (const ent of model.world.entities) {
      const isRepresented =
        ...
        Array.from(representedIds).some((id) => id.includes(ent.id) || ent.id.includes(id));
      ...
      for (const act of stepActions) {
        if (act.type === "highlight") {
          ...
          const entityExists = model.world.entities.some(
            (e) => e.id === target || target.includes(e.id) || e.id.includes(target),
          );
          if (!entityExists) {
            issues.push({
              stepIndex: i + 1,
              code: "INVALID_HIGHLIGHT_TARGET",
              message: `Visual highlight targets unknown entity ID '${target}' in step ${i + 1}.`,
              severity: "error",
            });
          }
        }
