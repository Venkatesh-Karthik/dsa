...
export function analyzeConcept(
  ...
  let cleanTitle = trimmed
    .replace(/^(explain|teach me|how does|what is|visualize|show me|walk me through)\s+/i, "")
    .replace(/\s+(step by step|visually|in depth|works?|algorithm|concept)\b/gi, "")
    .trim();
 ...
 */
export function compileConceptModelToVisualLesson(model: ConceptModel): VisualLesson {
  ...
  if (
    ...
    ((transformationsToMap[0] as any).stepIndex === 0 || transformationsToMap[0].fromStateIndex === 0) &&
    (!transformationsToMap[0].operations || transformationsToMap[0].operations.length === 0)
  ) {
  ...
  const transformations: Transformation[] = transformationsToMap.map((t) => {
    ...
    const state = model.states[t.toStateIndex] || model.states[t.fromStateIndex] || model.states[0];
    const inspectorData =
    ...
    return {
      ...
      insight: t.insight || (model.invariants[0] ? `Invariant: ${model.invariants[0].rule}` : undefined),
 ...
 */
export function extractConceptModelFromVisualLesson(lesson: VisualLesson): ConceptModel {
  if (lesson.conceptModel) {
    return lesson.conceptModel as ConceptModel;
  }
  const title = lesson.concept || lesson.title || lesson.topic || "Technical Concept";
  ...
  for (const act of actions) {
    if ("id" in act && typeof act.id === "string" && !entitySet.has(act.id)) {
      ...
      if ("label" in act && typeof act.label === "string") label = act.label;
      if ("text" in act && typeof act.text === "string") label = act.text;

      if (act.type === "create_tree") type = "TreeNode";
      else if (act.type === "create_array") type = "ArrayElement";
      else if (act.type === "create_linked_list") type = "LinkedListNode";
      else if (act.type === "create_graph") type = "GraphNode";
      else if (act.type === "create_circle") type = "State";
      else if (act.type === "create_box") type = "Component";
  ...
  for (let idx = 0; idx < rawTransformations.length; idx++) {
    ...
    const ct: ConceptTransformation = {
      ...
      reason: rt.explanation || "Progressing algorithm state according to transition logic.",
      ...
      insight: rt.insight,
      inspectorData: (rt as any).inspectorData || module.extractInspectorData(state),
    };
 ...
 * - Stable entity IDs
 */
export function validateConceptModel(model: ConceptModel): ConceptModelValidationResult {
  const errors: string[] = [];
