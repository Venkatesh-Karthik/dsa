...
export const DatabasesDomainModule: DomainKnowledgeModule = {
  ...
  description: "Relational Tables, SQL JOINs, Indexes, Transactions, and Query Execution Plans",
  ...
  suggestStrategy(concept: string, prompt: string = ""): TeachingStrategy {
    const text = `${concept} ${prompt}`.toLowerCase();
    if (text.includes("join") || text.includes("query") || text.includes("data flow")) {
      return "DATA_FLOW";
    }
    if (text.includes("transaction") || text.includes("acid") || text.includes("commit")) {
    ...
    return [
      {
        ...
        misunderstanding: "A SQL JOIN immediately filters matching rows during row loading without considering the cross product space.",
        misconception: "A SQL Cartesian product is the same as an INNER JOIN without filtering.",
        correction: "A logical join conceptualizes Cartesian product evaluation followed by predicate filter qualification.",
      },
      {
        id: "misc-left-join-null",
        misunderstanding: "LEFT JOIN drops left table rows when there is no matching right table record.",
        misconception: "LEFT JOIN drops left table rows when there is no matching right table record.",
        correction: "LEFT JOIN preserves all left table records; unmatched right columns are populated with NULL values.",
      },
    ];
  },
  ...
  extractInspectorData(
    ...
    if (metrics.length === 0) {
      ...
      const rows = activeEntities.filter(
        (e: any) => e.type === "Row" || e.semanticRole === "row" || e.semanticRole === "record",
      );
      ...
      const joinConditionMatch = allLabels.match(/(?:ON|where)\s+([\w.]+\s*=\s*[\w.]+)/i);

      if (tables.length > 0) {
        metrics.push({
          label: "Tables Involved",
          value: tables.map((t: any) => t.label?.split(" ")[0] || t.id).join(", "),
        });
      }
