...
export const MachineLearningDomainModule: DomainKnowledgeModule = {
  ...
  description: "Gradient Descent, Loss Functions, Weights, Biases, Backpropagation, and Neural Layers",
  ...
  suggestStrategy(concept: string, prompt: string = ""): TeachingStrategy {
    const text = `${concept} ${prompt}`.toLowerCase();
    if (text.includes("gradient") || text.includes("optimization") || text.includes("epoch")) {
    ...
    return [
      {
        ...
        misunderstanding: "A larger learning rate always accelerates convergence to the minimum.",
        misconception: "A larger learning rate always accelerates convergence to the minimum.",
        correction: "An excessively large learning rate causes catastrophic divergence or oscillating overshoots across the valley.",
      },
      {
        id: "misc-local-global-min",
        misunderstanding: "Gradient descent always converges to the absolute global minimum on non-convex surfaces.",
        misconception: "Gradient descent always converges to the absolute global minimum on non-convex surfaces.",
        correction: "Standard gradient descent can settle into local minima, saddle points, or plateaus depending on initialization.",
      },
    ];
  },
  ...
  extractInspectorData(
    ...
    if (metrics.length === 0) {
      metrics.push({ label: "Current Step", value: `Iteration ${stateIdx + 1}` });
      const lossVal = Math.max(0.01, +(1.0 / (stateIdx + 1.2)).toFixed(3));
      metrics.push({ label: "Loss (MSE)", value: lossVal, badgeColor: "#2563eb" });
    ...
    return {
      ...
      properties,
      sections,
      statusBadge: transformation?.inspectorData?.statusBadge || `Iteration ${stateIdx + 1}`,
