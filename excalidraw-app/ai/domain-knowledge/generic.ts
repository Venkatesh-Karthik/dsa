...
export const GenericDomainModule: DomainKnowledgeModule = {
  ...
  description: "Fallback domain intelligence for arbitrary, cross-disciplinary, or novel technical concepts",
  ...
  suggestStrategy(concept: string, prompt: string = ""): TeachingStrategy {
    ...
    if (text.includes("cycle") || text.includes("pipeline") || text.includes("circuit") || text.includes("stages")) {
      return "PIPELINE";
    }
    if (text.includes("flow") || text.includes("stream") || text.includes("data")) {
      return "DATA_FLOW";
    }
    if (text.includes("step") || text.includes("time") || text.includes("sequence")) {
      return "TEMPORAL_PROGRESSION";
    }
    if (text.includes("cause") || text.includes("reaction") || text.includes("effect")) {
    ...
    return [
      {
        ...
        misunderstanding: "A more complex diagram with dozens of overlapping boxes produces better understanding.",
        misconception: "A more complex diagram with dozens of overlapping boxes produces better understanding.",
        correction: "Effective learning relies on isolating 5-6 core entities and demonstrating their dynamic transformations.",
      },
      {
        id: "misc-generic-causality",
        misunderstanding: "Correlated visual state changes imply direct causality.",
        misconception: "Correlated visual state changes imply direct causality.",
        correction: "State progression requires explicit operations or forces driving the transition.",
      },
    ];
  },
  ...
  extractInspectorData(
    ...
    if (metrics.length === 0) {
      metrics.push({ label: "Phase", value: `Phase ${stateIdx + 1}` });
      metrics.push({ label: "Active Components", value: Math.max(1, activeEntities.length) });
      if (activeEntities.length > 0) {
        const primary = activeEntities[0];
        metrics.push({
          label: "Focus Component",
          value: primary.label || primary.name || primary.id,
          badgeColor: "#2563eb",
        });
      }
    }
    ...
    return {
      ...
      properties,
      sections,
      statusBadge: transformation?.inspectorData?.statusBadge || `Phase ${stateIdx + 1}`,
