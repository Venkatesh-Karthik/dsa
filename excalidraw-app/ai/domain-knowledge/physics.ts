...
export const PhysicsDomainModule: DomainKnowledgeModule = {
  ...
  description: "Kinematics, Newtonian Forces, Momentum, Work-Energy Theorem, and Projectile Motion",
  ...
  suggestStrategy(concept: string, prompt: string = ""): TeachingStrategy {
    const text = `${concept} ${prompt}`.toLowerCase();
    if (text.includes("projectile") || text.includes("trajectory") || text.includes("time") || text.includes("motion")) {
    ...
    return [
      {
        ...
        misunderstanding: "An object moving at constant velocity requires a continuous forward net force.",
        misconception: "An object moving at constant velocity requires a continuous forward net force.",
        correction: "Newton's First Law: A constant velocity object has net force = 0; force is only required to accelerate or decelerate.",
      },
      {
        id: "misc-projectile-mass",
        misunderstanding: "Heavier objects fall faster in a vacuum than lighter objects.",
        misconception: "Newton Third Law action reaction forces cancel each other out on the same body.",
        correction: "Gravitational acceleration g is identical for all masses in vacuum independent of object mass. Action and reaction forces act on different bodies.",
      },
    ];
  },
  ...
  extractInspectorData(
    ...
    if (metrics.length === 0) {
      ...
      metrics.push({ label: "Active Bodies", value: Math.max(1, activeEntities.length) });
      ...
      if (projectile) {
        metrics.push({ label: "Mass / Body", value: projectile.label || "Mass m", badgeColor: "#2563eb" });
      }
      metrics.push({ label: "Time Elapsed", value: `t = ${(stateIdx * 0.5).toFixed(1)}s` });
    }
    ...
    return {
      ...
      properties,
      sections,
      statusBadge: transformation?.inspectorData?.statusBadge || `t = ${(stateIdx * 0.5).toFixed(1)}s`,
