...
export const DsaDomainModule: DomainKnowledgeModule = {
  ...
  description:
    "Trees, Graphs, Arrays, Linked Lists, Stacks, Queues, Heaps, Dynamic Programming, Sorting & Searching",
    ...
    if (text.includes("avl") || text.includes("bst") || text.includes("tree")) {
      invariants.push({
        ...
        check: (state, model) => {
          ...
          const activeRels = (model.relationships || []).filter((r) =>
            state.activeRelationshipIds?.includes(r.id),
          );
          ...
          const rootId =
            state.activeEntityIds.find((id) => !hasParent.has(id)) ||
            state.activeEntityIds[0];
          if (!rootId) {
            return true;
          }
          ...
          function validateSubtree(
            ...
            if (!Number.isNaN(val)) {
              ...
              if (left && !validateSubtree(left, min, val)) {
                return false;
              }
              const right = rightChildren.get(nodeId);
              if (right && !validateSubtree(right, val, max)) {
                return false;
              }
            }
            return true;
        ...
        invariants.push({
          ...
          check: (state, model) => {
            ...
            function getHeight(id?: string): number {
              if (!id || !state.activeEntityIds.includes(id)) {
                return 0;
              }
              return (
                1 +
                Math.max(
                  getHeight(leftChildren.get(id)),
                  getHeight(rightChildren.get(id)),
                )
              );
            }
      ...
      invariants.push({
        ...
        check: (state, model) => {
          ...
          const isDoubly =
            text.includes("doubly") ||
            text.includes("double") ||
            activeRels.some(
              (r) =>
                r.type === "previous" || r.type === "prev" || r.type === "back",
            );
          const outDegree = new Map<string, number>();
          for (const r of activeRels) {
            const relType = (r.type || "next").toLowerCase();
            const key = isDoubly
              ? `${r.sourceEntityId}:${relType}`
              : r.sourceEntityId;
            outDegree.set(key, (outDegree.get(key) || 0) + 1);
          }
          // A singly linked list has at most 1 outgoing pointer per node.
          // A doubly linked list has at most 1 outgoing pointer per relation type (next, previous) per node.
  ...
  getMisconceptions(concept: string): ConceptMisconception[] {
    ...
    if (text.includes("avl") || text.includes("rotation")) {
      list.push({
        id: "misc-avl-inorder",
        misunderstanding:
          "Rotation reorganizes elements and changes inorder sorted traversal.",
        misconception:
          "Rotation reorganizes elements and changes inorder sorted traversal.",
        correction:
          "Rotation changes tree geometry to reduce height, but strictly preserves BST inorder key order.",
      });
      list.push({
        id: "misc-avl-pivot",
        misunderstanding:
          "The imbalanced node rotates alone without child rebinding.",
        misconception:
          "The imbalanced node rotates alone without child rebinding.",
        correction:
          "The pivot node becomes the new root; its inner child is handed off to the old root.",
      });
    } else if (text.includes("linked list")) {
      list.push({
        id: "misc-ll-insert",
        misunderstanding:
          "Creating a node object is identical to inserting it into the list.",
        misconception:
          "Creating a node object is identical to inserting it into the list.",
        correction:
          "An allocated node remains an orphan until incoming and outgoing pointer links are wired.",
      });
    } else if (text.includes("binary search")) {
      list.push({
        id: "misc-bs-unsorted",
        misunderstanding:
          "Binary search can locate elements in any arbitrary list.",
        misconception:
          "Binary search can locate elements in any arbitrary list.",
        correction:
          "Binary search fundamentally relies on monotonic sorted order to eliminate half the space.",
      });
    } else if (text.includes("bfs")) {
      list.push({
        ...
        correction:
          "BFS visits nodes strictly by topological frontier depth from the starting vertex.",
      });
    }
    ...
    if (list.length === 0) {
      list.push({
        ...
        misunderstanding:
          "Choosing a complex data structure always yields higher performance.",
        misconception:
          "Choosing a complex data structure always yields higher performance.",
        correction:
          "Simpler contiguous structures like dynamic arrays often outperform linked nodes due to hardware cache locality.",
      });
    }
  ...
  extractInspectorData(
    ...
    if (metrics.length === 0) {
      ...
      const pivotOrTarget = activeEntities.find(
        (e: any) =>
          e.semanticRole === "pivot" ||
          e.semanticRole === "target" ||
          e.semanticRole === "active",
      );
      ...
      if (pivotOrTarget) {
        metrics.push({
          label:
            pivotOrTarget.semanticRole === "pivot" ? "Pivot Node" : "Target",
          value: String(
            pivotOrTarget.value ?? pivotOrTarget.label ?? pivotOrTarget.id,
          ),
          badgeColor: "#2563eb",
        });
      }
    ...
    return {
      ...
      sections,
      statusBadge:
        transformation?.inspectorData?.statusBadge || `State ${stateIdx + 1}`,
