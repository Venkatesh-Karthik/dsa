...
export const NetworkingDomainModule: DomainKnowledgeModule = {
  ...
  description:
    "HTTP, TCP/IP, DNS, Sockets, Packets, Client-Server Architectures, and REST APIs",
  ...
  getMisconceptions(concept: string): ConceptMisconception[] {
    ...
    if (text.includes("http") || text.includes("api")) {
      list.push({
        ...
        correction:
          "The API is an interface/transport contract; the server processes the request and accesses storage independently.",
      });
      list.push({
        id: "misc-http-state",
        misunderstanding:
          "HTTP retains client connection memory by default between distinct requests.",
        misconception:
          "HTTP/1.1 is inherently stateless at the protocol layer; session state is managed via tokens/cookies.",
        correction:
          "HTTP/1.1 is inherently stateless at the protocol layer; session state is managed via tokens/cookies.",
      });
    }
    if (
      ...
      list.push({
        id: "misc-tcp-synack",
        misunderstanding:
          "A TCP connection is active immediately when the client sends the initial SYN packet.",
        misconception:
          "A TCP handshake creates an active connection immediately when the client sends the initial SYN packet.",
        correction:
          "TCP requires 3 steps: SYN from client, SYN-ACK from server, and final ACK from client before data flows.",
      });
    }
    ...
    if (list.length === 0) {
      list.push({
        ...
        misunderstanding:
          "Bandwidth increases automatically eliminate network request latency.",
        misconception:
          "Bandwidth increases automatically eliminate network request latency.",
        correction:
          "Round-trip propagation delay (RTT) is bounded by the speed of light and physical distance, independent of throughput.",
      });
    }
  ...
  extractInspectorData(
    ...
    if (metrics.length === 0) {
      ...
      const statusMatch = allLabels.match(
        /\b(200(\s*OK)?|201|400|401|404|500)\b/i,
      );

      if (client) {
        metrics.push({ label: "Client", value: client.label || "Browser" });
      }
      if (server) {
        metrics.push({
          label: "Server",
      ...
      if (methodMatch) {
        metrics.push({
          label: "Method",
          value: methodMatch[1].toUpperCase(),
          badgeColor: "#2563eb",
        });
      }
      if (statusMatch) {
        metrics.push({
          label: "Status",
          value: statusMatch[1].toUpperCase(),
          badgeColor: "#16a34a",
        });
      }
      metrics.push({
        ...
        value:
          ...
          state?.activeRelationshipIds?.length ||
          1,
      });
    }
