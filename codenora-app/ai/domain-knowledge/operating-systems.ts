...
export const OperatingSystemsDomainModule: DomainKnowledgeModule = {
  ...
  description:
    "Processes, Threads, Scheduling, Virtual Memory, Deadlocks, and Concurrency",
    ...
    return [
      {
        ...
        misunderstanding:
          "A single core runs multiple threads simultaneously in true physical parallel.",
        misconception:
          "A single core runs multiple threads simultaneously in true physical parallel.",
        correction:
          "On a single core, threads time-slice via rapid context switches; true parallelism requires multiple physical cores.",
      },
      {
        id: "misc-deadlock",
        ...
        correction:
          "Deadlock is caused by circular resource waiting conditions, irrespective of CPU workload.",
      },
    ];
  },
  ...
  extractInspectorData(
    ...
    if (metrics.length === 0) {
      ...
      metrics.push({
        label: "Ready Queue",
        value:
          readyQueue.length > 0 ? `${readyQueue.length} processes` : "Empty",
      });
      metrics.push({
