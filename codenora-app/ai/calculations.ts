  ...
  if (treeAction && treeAction.nodes && treeAction.root) {
    ...
    if (treeMetrics.imbalancedNodes.length > 0) {
      const imbalancedList = treeMetrics.imbalancedNodes
        .map(
          (n) =>
            `Node ${n.value}: Left h=${n.leftHeight}, Right h=${
              n.rightHeight
            } → BF = ${n.balanceFactor > 0 ? "+" : ""}${n.balanceFactor}`,
        )
        .join(" | ");
      ...
      if (rootMetric) {
        verifiedCalculations.push(
          `Tree Height = ${treeMetrics.treeHeight} | Root ${
            rootMetric.value
          } BF = ${rootMetric.balanceFactor > 0 ? "+" : ""}${
            rootMetric.balanceFactor
          } (Balanced: |BF| ≤ 1)`,
        );
      }
    }
