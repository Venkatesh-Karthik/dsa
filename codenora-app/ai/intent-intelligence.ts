...
export function classifyDetailedIntent(
  ...
  // Dimensions
  const isComparison =
    /\b(compare|vs\.?|versus|difference between|trade-?offs?|contrast)\b/i.test(
      lower,
    );
  const isWhatIf =
    /\b(what if|what happens if|suppose we change|if we change|modify|mutate|swap)\b/i.test(
      lower,
    );
  const isCausal =
    /\b(why|how come|cause of|reason for|why does|what makes)\b/i.test(lower);
  const isIntuition =
    /\b(intuition|intuitive|mental model|in simple terms|eli5|high level|big picture)\b/i.test(
      lower,
    );
  const isImplementation =
    /\b(code|implementation|implement|syntax|program|write (a|the) (code|function))\b/i.test(
      lower,
    );
  const isMath =
    /\b(math|mathematical|formula|equation|proof|derive|derivation|complexity|o\(|theta\()\b/i.test(
      lower,
    );
  const isPractice =
    /\b(practice|quiz|test me|exercise|question|challenge|verify my)\b/i.test(
      lower,
    );
  const isDebugging =
    /\b(bug|fix|wrong|incorrect|doesn't work|failing|error|confusion|stuck)\b/i.test(
      lower,
    );
  ...
  if (/\b(briefly|quick|overview|summary|tldr|nutshell)\b/i.test(lower)) {
    ...
    /\b(deep|in-?depth|thorough|detailed|rigorous|comprehensive|step by step)\b/i.test(
      lower,
    )
  ) {
    depth = "deep";
  } else if (
    /\b(exhaustive|every detail|all cases|under the hood|internals)\b/i.test(
      lower,
    )
  ) {
    depth = "exhaustive";
  ...
  let targetConcept = text
    .replace(
      /^(explain(\s+how(\s+an?)?)?|teach\s+me(\s+how(\s+an?)?)?|how\s+does|what\s+is|visualize|show\s+me|walk\s+me\s+through|compare|why\s+does)\s+/i,
      "",
    )
    .replace(
      /\s+(step by step|visually|in depth|works?|algorithm|concept|in simple terms)\b/gi,
      "",
    )
    .trim();
  ...
  targetConcept =
    targetConcept.charAt(0).toUpperCase() + targetConcept.slice(1);
