...
export class LatencyTracker {
  ...
  mark<K extends keyof LatencyTimestamps>(
    stage: K,
  ...
  getBreakdown(): LatencyBreakdown {
    ...
    const layoutEngineMs =
      t.t5_state_reconstructed && t.t6_layout_computed
        ? Math.max(
            0,
            Math.round(t.t6_layout_computed - t.t5_state_reconstructed),
          )
        : undefined;
  ...
  logSummary(topic?: string): void {
    ...
    console.groupCollapsed(
      `[Cognora Latency] ${topic ? `"${topic}"` : "Lesson"} - Total: ${
        breakdown.totalMs
      }ms`,
    );
    // eslint-disable-next-line no-console
    console.table({
      "T0->T1 Client Dispatch": breakdown.clientDispatchMs
        ? `${breakdown.clientDispatchMs} ms`
        : "N/A",
      "T1->T2 AI Provider Completion": breakdown.aiProviderMs
        ? `${breakdown.aiProviderMs} ms`
        : "N/A",
      "T3->T4 DSL Validation & Repair": breakdown.validationMs
        ? `${breakdown.validationMs} ms`
        : "N/A",
      "T4->T5 Semantic State Rebuild": breakdown.stateEngineMs
        ? `${breakdown.stateEngineMs} ms`
        : "N/A",
      "T5->T6 Layout Engine": breakdown.layoutEngineMs
        ? `${breakdown.layoutEngineMs} ms`
        : "N/A",
      "T6->T7 Canvas Render & Focus": breakdown.canvasRenderMs
        ? `${breakdown.canvasRenderMs} ms`
        : "N/A",
