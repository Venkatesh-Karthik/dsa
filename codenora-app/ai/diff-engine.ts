...
export function computeSceneGraphDiff(
  ...
  for (const [id, toEntity] of toEntities.entries()) {
    ...
    if (!fromEntity) {
      ...
      const positionChanged =
        fromPos && toPos
          ? Math.abs(fromPos.x - toPos.x) > 0.5 ||
      ...
      if (
        ...
        stateChanged ||
        highlightChanged ||
        JSON.stringify(fromEntity.properties) !==
          JSON.stringify(toEntity.properties)
      ) {
  ...
  if (addedEntities.length > 0) {
    summaryParts.push(`+${addedEntities.length} entities`);
  }
  if (removedEntities.length > 0) {
    summaryParts.push(`-${removedEntities.length} entities`);
  }
  if (updatedEntities.length > 0) {
    summaryParts.push(`~${updatedEntities.length} entities`);
  }
  if (addedRelationships.length > 0) {
    summaryParts.push(`+${addedRelationships.length} edges`);
  }
  if (removedRelationships.length > 0) {
    summaryParts.push(`-${removedRelationships.length} edges`);
  }
  ...
  return {
    ...
    removedAnnotations,
    summary:
      summaryParts.length > 0 ? summaryParts.join(", ") : "No semantic changes",
  };
}
