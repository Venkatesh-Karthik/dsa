...
export function inspectSelectedEntity(
  ...
  const currentT =
    model.transformations[currentStepIndex] || model.transformations[0];
  ...
  for (let i = currentStepIndex + 1; i < model.transformations.length; i++) {
    ...
    if (nextT.relevantEntityIds.includes(entityId)) {
      nextChangeSummary = `In Step ${i + 1} ("${nextT.title}"): ${
        nextT.action
      }`;
      break;
    }
...
export function deriveWhatChangedExplanation(
  ...
  if (fromState && toState) {
    ...
    if (diff.addedEntities.length > 0) {
      parts.push(
        `Added: ${diff.addedEntities.map((e) => e.label || e.id).join(", ")}`,
      );
    }
    if (diff.removedEntities.length > 0) {
      parts.push(
        `Removed: ${diff.removedEntities
          .map((e) => e.label || e.id)
          .join(", ")}`,
      );
    }
    if (diff.updatedEntities.length > 0) {
      const updates = diff.updatedEntities.map((u) => {
        if (u.valueChanged) {
          return `${u.id} value -> ${u.to.value}`;
        }
        if (u.stateChanged) {
          return `${u.id} state -> ${u.to.state}`;
        }
        if (u.highlightChanged) {
          return `${u.id} highlighted`;
        }
        return `${u.id} updated`;
      });
    ...
    if (diff.addedRelationships.length > 0) {
      parts.push(
        `Connected: ${diff.addedRelationships
    ...
    if (diff.removedRelationships.length > 0) {
      parts.push(
        `Disconnected: ${diff.removedRelationships
          .map((r) => `${r.sourceEntityId} -x- ${r.targetEntityId}`)
          .join(", ")}`,
      );
    }
...
export function deriveWhyExplanation(
  ...
  if (model.causalRelationships) {
    ...
    if (relevantCausal) {
      parts.push(
        `Direct causal rule: ${relevantCausal.cause} -> ${relevantCausal.effect} (${relevantCausal.mechanism})`,
      );
    }
  }
  ...
  return (
    ...
    "Advances algorithmic state toward target invariant."
  );
}
  ...
  if (!primaryInvariant) {
    return {
      ...
      counterexampleScenario:
        "An operation modifies downstream connections before establishing upstream references.",
      observableConsequence:
        "Memory leakage or unreachable orphaned sub-elements occur.",
    };
  }
...
export function evaluateParameterMutation(
  ...
  const matchingEntity = model.entities.find(
    (e) =>
      e.label.toLowerCase() === paramName.toLowerCase() || e.id === paramName,
  );
  ...
  if (matchingEntity) {
    ...
    for (const rel of model.relationships) {
      if (rel.sourceEntityId === matchingEntity.id) {
        affected.push(rel.targetEntityId);
      }
      if (rel.targetEntityId === matchingEntity.id) {
        affected.push(rel.sourceEntityId);
      }
    }
  }
  ...
  return {
    ...
    semanticPrediction: `Mutating ${paramName} to ${String(newValue)} affects ${
      affected.length
    } connected entities and triggers re-evaluation of invariants.`,
  };
}
...
export function derivePracticeItem(
  ...
  if (model.practiceOpportunities && model.practiceOpportunities.length > 0) {
    const opp =
      model.practiceOpportunities[
        stepIndex % model.practiceOpportunities.length
      ];
    return {
      id: opp.id,
      question: opp.prompt,
  ...
  if (nextT) {
    return {
      ...
      explanation:
        nextT.reason ||
        `Following "${t.title}", ${nextT.action} is necessary to advance toward completion.`,
    };
  }
  ...
  return {
    ...
    options: [
      model.invariants[0]?.rule ||
        "Every entity reference must point to a valid target",
    ...
    correctIndex: 0,
    explanation:
      model.invariants[0]?.description ||
      "Invariant rules guarantee structural and logical consistency.",
  };
}
