...
export function extractVisualRequirements(params: {
  understanding?: Partial<QuestionUnderstandingResult> & {
    concept?: string;
    [key: string]: any;
  };
  ...
  const currentState =
    model.states[stepIndex] || model.states[0] || model.world.states[0];
  ...
  const hasCycles =
    ...
    (blueprint?.causalChain || []).some((c) =>
      (blueprint?.causalChain || []).some(
        (c2) => c2.sourceId === c.targetId && c2.targetId === c.sourceId,
      ),
    );

  const entArray = currentState
    ? Array.from(currentState.entities.values())
    : model.world.entities;
  const relArray = currentState
  ...
  const hasDecisions =
    ...
    model.states.some(
      (s) => s.activeDecision !== undefined || s.stateType === "decision",
    );
  ...
  const semCtx: SemanticContext = {
    ...
    hasTreeHierarchy,
    hasOrderedSequence:
      understanding?.scope === "interaction" ||
      understanding?.scope === "end-to-end",
    hasDecisions,
    hasMemoryPointers,
    hasTabularData,
    hasScientificRays,
    hasCallStack,
    hasInputOutput: /\b(input|output|encoder|decoder|transform)\b/i.test(
      concept,
    ),
  ...
  const layoutStrategy: SupportedLayoutStrategy =
    matchedCompound.supportedLayout;
  ...
  for (const ent of entArray) {
    ...
    if (ent.id === primaryFocusId || isAffected) {
      ...
      ent.semanticRole === "actor" ||
      ent.semanticRole === "server"
    ) {
      importance = "primary";
    } else if (
      ent.semanticRole === "background" ||
      ent.properties?.background === true
    ) {
    ...
    let visualType = "Node";
    if (ent.type === "TreeNode" || hasTreeHierarchy) {
      visualType = "TreeNode";
    } else if (ent.type === "ArrayCell" || /\barray\b/i.test(concept)) {
      visualType = "ArrayCell";
    } else if (ent.type === "Client" || /\bclient\b/i.test(ent.id)) {
      visualType = "Client";
    } else if (ent.type === "Server" || /\bserver\b/i.test(ent.id)) {
      visualType = "Server";
    } else if (ent.type === "CallFrame" || hasCallStack) {
      visualType = "CallFrame";
    } else if (ent.type === "MemoryBlock" || hasMemoryPointers) {
      visualType = "MemoryBlock";
    } else if (ent.type === "Table" || hasTabularData) {
      visualType = "Table";
    } else if (ent.type === "Ray" || hasScientificRays) {
      visualType = "Ray";
    ...
    const isPersistent =
      ent.properties?.persistence !== "temporary" &&
  ...
  for (const rel of relArray) {
    let relImportance: "primary" | "secondary" | "temporary" | "background" =
      "secondary";
    if (
  ...
  return {
    ...
    requiredStates: model.states.map((s, idx) => ({
      ...
      decisionBadge: s.activeDecision
        ? `Decision: ${s.activeDecision.condition}`
        : undefined,
      invariantBadges: (model.invariants || [])
        .slice(0, 2)
