...
export function extractSelectedElementsContext(
  ...
  for (const id of selectedSet) {
    ...
    let parentStructureId: string | undefined = el.customData?.structureId as
      | string
      | undefined;
...
export function formatSelectedElementChip(
  ...
  const rawName = first.label || first.dslId;
  const cleanName = ExplanationEngine.scrubMetadata(rawName);
  const name = cleanName ? `"${cleanName}"` : `"Element"`;
