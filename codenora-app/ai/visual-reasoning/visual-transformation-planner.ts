...
export class VisualTransformationPlanner {
  ...
  public static plan(
    ...
    for (let i = 0; i < transformations.length; i++) {
      ...
      const nextState =
        states[trans.toStateIndex] || states[i + 1] || prevState;
      ...
      if (mutated.length > 0 && !explanation.includes(mutated[0])) {
        const entLabel =
          nextState.entities.get(mutated[0])?.label || mutated[0];
      ...
      steps.push({
        ...
        decisionEvaluation: trans.decision
          ? {
              ...
              outcome:
                trans.selectedOutcome || trans.decision.selectedOutcomeId,
            }
          : undefined,
      });
    }
