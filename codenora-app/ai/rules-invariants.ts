...
export function evaluateAcyclic(state: SemanticState): {
  holds: boolean;
  details?: string;
} {
  ...
  for (const node of state.entities.keys()) {
    if (!visited.has(node)) {
      if (hasCycle(node)) {
        return {
          holds: false,
          details: `Cycle detected in graph starting from entity '${node}'`,
        };
      }
    }
  }
...
export function evaluateBST(state: SemanticState): {
  holds: boolean;
  details?: string;
} {
  ...
  for (const rel of state.relationships.values()) {
    const isLeft =
      ...
      rel.type === "leftOf" ||
      rel.label === "L" ||
      rel.properties?.role === "left";
    const isRight =
      rel.type === "right" ||
      rel.type === "rightOf" ||
      rel.label === "R" ||
      rel.properties?.role === "right";
    ...
    if (isLeft) {
      ...
      if (
        sourceEnt &&
        targetEnt &&
        sourceEnt.value !== undefined &&
        targetEnt.value !== undefined
      ) {
  ...
  function validateNode(
    ...
    if (!Number.isNaN(num)) {
      if (num <= min || num >= max) {
        return {
          valid: false,
          error: `BST ordering violated at node '${
            ent.label || nodeId
          }' with value ${num} (expected range (${min}, ${max}))`,
        };
      }
...
export function evaluateAVL(state: SemanticState): {
  holds: boolean;
  details?: string;
} {
  const leftChildren = new Map<string, string>();
  ...
  function getHeight(nodeId?: string): number {
    if (!nodeId || !state.entities.has(nodeId)) {
      return 0;
    }
  ...
  for (const nodeId of state.entities.keys()) {
    const isTreeParticipant =
      leftChildren.has(nodeId) ||
      rightChildren.has(nodeId) ||
      hasParent.has(nodeId);
    if (!isTreeParticipant && state.entities.size > 3) {
      continue;
    }
    ...
    if (diff > 1) {
      ...
      return {
        holds: false,
        details: `AVL balance violated at node '${
          ent?.label || nodeId
        }': left height ${hLeft}, right height ${hRight}, balance factor ${diff} > 1`,
      };
    }
  }
...
export function evaluateConservation(
  ...
  for (const ent of initial.entities.values()) {
    if (typeof ent.value === "number") {
      initialTotal += ent.value;
    }
  }
  for (const ent of state.entities.values()) {
    if (typeof ent.value === "number") {
      currentTotal += ent.value;
    }
  }
  ...
  for (const [id, ent] of state.entities.entries()) {
    if (typeof ent.value === "number") {
      if (ent.value < min || ent.value > max) {
        return {
          holds: false,
          details: `Bounds violated at '${ent.label || id}': ${
            ent.value
          } not in [${min}, ${max}]`,
        };
      }
...
/**
 * Evaluates whether an ordered collection of entities (array, linked list, sequence)
 * maintains the required ordering (e.g., ascending or descending).
 */
export function evaluateOrdering(
  state: SemanticState,
  order: "ascending" | "descending" = "ascending",
): { holds: boolean; details?: string } {
  // Collect entities that belong to sequences or have index properties
  const sequenceEntities: Array<{
    id: string;
    label: string;
    value: number;
    index: number;
  ...
  for (const [id, ent] of state.entities.entries()) {
    const rawVal = ent.value !== undefined ? ent.value : Number(ent.label);
    const num = typeof rawVal === "number" ? rawVal : Number(rawVal);
    const idx =
      typeof ent.properties?.index === "number"
        ? ent.properties.index
        : undefined;
  ...
  // Sort by index
  sequenceEntities.sort((a, b) => a.index - b.index);

  if (sequenceEntities.length > 1) {
    for (let i = 0; i < sequenceEntities.length - 1; i++) {
      const curr = sequenceEntities[i];
      const next = sequenceEntities[i + 1];
      if (order === "ascending" && curr.value > next.value) {
        return {
          holds: false,
          details: `Ordering invariant violated: element '${curr.label}' (${curr.value}) at index ${curr.index} > '${next.label}' (${next.value}) at index ${next.index}`,
        };
      } else if (order === "descending" && curr.value < next.value) {
        return {
          holds: false,
          details: `Ordering invariant violated: element '${curr.label}' (${curr.value}) at index ${curr.index} < '${next.label}' (${next.value}) at index ${next.index}`,
        };
...
/**
 * Evaluates whether values, indices, or pointers fall within their valid allowable range.
 */
export function evaluateRangeValidity(
  state: SemanticState,
  min = -Infinity,
  max = Infinity,
): { holds: boolean; details?: string } {
  // Check bounds of values
  const boundsRes = evaluateBounds(state, min, max);
  if (!boundsRes.holds) {
    return boundsRes;
  }

  // Check array and pointer index validity
  let maxArrayLen = 0;
  for (const ent of state.entities.values()) {
    if (
      ent.type !== "Pointer" &&
      ent.semanticRole !== "pointer" &&
      typeof ent.properties?.index === "number"
    ) {
      maxArrayLen = Math.max(maxArrayLen, ent.properties.index + 1);
    }
  }

  for (const [id, ent] of state.entities.entries()) {
    if (ent.type === "Pointer" || ent.semanticRole === "pointer") {
      const ptrIndex = ent.properties?.index ?? ent.value;
      if (typeof ptrIndex === "number" && maxArrayLen > 0) {
        if (ptrIndex < 0 || ptrIndex >= maxArrayLen) {
          return {
            holds: false,
            details: `Pointer index out of range: '${
              ent.label || id
            }' points to index ${ptrIndex}, valid range is [0, ${
              maxArrayLen - 1
            }]`,
          };
        }
...
/**
 * Evaluates reference validity: pointers, links, and relationships must reference existing entities.
 */
export function evaluateReferenceValidity(state: SemanticState): {
  holds: boolean;
  details?: string;
} {
  for (const rel of state.relationships.values()) {
    if (!state.entities.has(rel.source)) {
      return {
        holds: false,
        details: `Invalid reference: relationship '${rel.id}' source '${rel.source}' does not exist`,
      };
    }
    if (!state.entities.has(rel.target)) {
      return {
        holds: false,
        details: `Invalid reference: relationship '${rel.id}' target '${rel.target}' does not exist`,
      };
    }
  }
  ...
  for (const [id, ent] of state.entities.entries()) {
    if (
      typeof ent.properties?.targetId === "string" &&
      !state.entities.has(ent.properties.targetId)
    ) {
      return {
        holds: false,
        details: `Dangling entity reference: '${
          ent.label || id
        }' targets non-existent entity '${ent.properties.targetId}'`,
      };
    }
  }
...
/**
 * Evaluates universal mathematical constraints, conservation laws, and equilibrium formulas.
 */
export function evaluateMathematicalConstraints(
  state: SemanticState,
  history?: SemanticState[],
): { holds: boolean; details?: string } {
  // 1. Non-negativity check for physical or count quantities
  for (const [id, ent] of state.entities.entries()) {
    if (
      ent.type === "Mass" ||
      ent.type === "Energy" ||
      ent.type === "Count" ||
      ent.type === "Distance" ||
      ent.properties?.nonNegative === true
    ) {
      if (typeof ent.value === "number" && ent.value < 0) {
        return {
          holds: false,
          details: `Mathematical constraint violated: quantity '${
            ent.label || id
          }' cannot be negative (${ent.value} < 0)`,
        };
      }
    }
  }

  // 2. Probability distribution sum to 1
  const probabilities: number[] = [];
  for (const ent of state.entities.values()) {
    if (ent.type === "Probability" || ent.properties?.isProbability === true) {
      if (typeof ent.value === "number") {
        probabilities.push(ent.value);
      }
    }
  }
  if (probabilities.length > 1) {
    const sum = probabilities.reduce((acc, v) => acc + v, 0);
    if (Math.abs(sum - 1.0) > 0.05) {
      return {
        holds: false,
        details: `Probability constraint violated: distribution sum is ${sum.toFixed(
          3,
        )} (expected 1.0)`,
      };
    }
  ...
  // 3. Conservation across history if applicable
  if (history && history.length > 0) {
    return evaluateConservation(state, history);
  }
...
export function evaluateTransactionIntegrity(state: SemanticState): {
  holds: boolean;
  details?: string;
} {
  ...
  for (const ent of state.entities.values()) {
    if (
      ent.type?.toLowerCase().includes("transaction") ||
      ent.semanticRole === "transaction"
    ) {
...
export function evaluatePreconditions(
  ...
  for (const p of preconditions) {
    ...
    const entityMatch = p.match(
      /(?:entity\s+|node\s+|component\s+)?['"]?([a-zA-Z0-9_-]+)['"]?\s+(?:must\s+)?(?:exists?|is\s+present|ready)/i,
    );
    if (entityMatch && entityMatch[1]) {
      const entName = entityMatch[1];
      const hasEnt = Array.from(state.entities.values()).some(
        (e) =>
          e.id === entName || e.label.toLowerCase() === entName.toLowerCase(),
      );
      if (!hasEnt) {
        failures.push(
          `Precondition failed: required entity '${entName}' is not present in state ${state.index}`,
        );
        continue;
      }
    }
    ...
    const statusMatch = p.match(
      /['"]?([a-zA-Z0-9_-]+)['"]?\s+(?:is\s+in\s+state|status\s+is|is)\s+['"]?([a-zA-Z0-9_-]+)['"]?/i,
    );
    if (statusMatch && statusMatch[1] && statusMatch[2]) {
      ...
      if (
        ent &&
        ent.state &&
        ent.state.toLowerCase() !== expectedState &&
        !["completed", "active", "ready", "normal"].includes(expectedState)
      ) {
        failures.push(
          `Precondition failed: entity '${ent.label}' state '${ent.state}' != expected '${expectedState}'`,
        );
        continue;
      }
    }
...
export function evaluatePostconditions(
  ...
  for (const p of postconditions) {
    ...
    const assignMatch = p.match(
      /['"]?([a-zA-Z0-9_-]+)['"]?\s*(?:=|is|becomes|reaches)\s*([₹$]?\d+(?:\.\d+)?|[a-zA-Z0-9_-]+)/i,
    );
      ...
      if (ent && ent.value !== undefined) {
        ...
        if (!Number.isNaN(numExpected) && !Number.isNaN(numVal)) {
          if (Math.abs(numVal - numExpected) > 0.001) {
            failures.push(
              `Postcondition failed: '${ent.label}' value ${ent.value} does not match expected ${assignMatch[2]}`,
            );
            continue;
          }
...
export function explainInvariantViolation(
  ...
  let whyInvalid = `The system entered state ${state.index} (${
    state.name || state.id
  }) which violates constraint "${violation.statement}".`;
  let consequence =
    "The system cannot commit or consider this state valid without restorative intervention.";
  let restorationAction =
    "Apply a compensating transformation, rollback, or rebalancing operation to restore invariant validity.";

  const text = `${violation.statement} ${details}`.toLowerCase();
  if (text.includes("balance") || text.includes("avl")) {
    whyInvalid =
      "The height differential between left and right subtrees exceeds the allowable balance threshold factor of 1.";
    consequence =
      "Search and insertion operations degrade from logarithmic O(log N) toward linear O(N) complexity.";
    restorationAction =
      "Perform tree rotation (LL, RR, LR, or RL) on the critical unbalance pivot node.";
  } else if (text.includes("bst") || text.includes("binary search tree")) {
    whyInvalid =
      "A node key violates the Binary Search Tree ordering property (left < root < right).";
    consequence =
      "Binary search lookups will fail to find existing keys or branch into wrong subtrees.";
    restorationAction =
      "Relocate the misplaced node to its correct BST position or adjust parent pointers.";
  } else if (
    text.includes("order") ||
    ...
    whyInvalid =
      "The sequential ordering constraint is violated between adjacent elements in the sequence.";
    consequence =
      "Sorted sequence guarantees, binary search, or priority scheduling are invalidated.";
    restorationAction =
      "Reorder or swap the misordered elements to restore valid monotonic sequence order.";
  } else if (
    text.includes("range") ||
    text.includes("boundary") ||
    text.includes("pointer index")
  ) {
    whyInvalid =
      "An entity value or pointer index exceeded its allowable domain boundary.";
    consequence =
      "Out-of-bounds access, illegal memory reference, or unnormalized value occurs.";
    restorationAction =
      "Clamp value to boundary limits or validate pointer termination bounds.";
  } else if (
    ...
    whyInvalid =
      "A connector or pointer references an entity ID that does not exist in the active semantic world.";
    consequence =
      "Graph traversal, message routing, or pointer dereference reaches undefined endpoints.";
    restorationAction =
      "Re-anchor the connector to a valid entity ID or prune obsolete disconnected relationships.";
  } else if (
    text.includes("conservation") ||
    text.includes("atomicity") ||
    text.includes("total") ||
    text.includes("money") ||
    text.includes("energy")
  ) {
    whyInvalid =
      "Total conserved value (energy, balance, mass, or resource count) changed without an external input/output conduit.";
    consequence =
      "Conservation integrity broken; resources were created or destroyed illegally.";
    restorationAction =
      "Execute rollback operation to restore the last verified checkpoint state or apply inverse compensating transaction.";
  } else if (
    ...
    whyInvalid =
      "Mathematical constraint, non-negativity rule, or probability normalization condition was violated.";
    consequence =
      "System model yields physically impossible or mathematically non-convergent results.";
    restorationAction =
      "Re-evaluate governing equation steps or normalize parameter values.";
  }
...
export function synthesizeConstraintEvaluator(
  ...
  if (
    ...
    text.includes("search tree order")
  ) {
  ...
  if (
    ...
    text.includes("balanced factor")
  ) {
    return (state) => evaluateAVL(state);
  }

  if (
    text.includes("acyclic") ||
    text.includes("no cycle") ||
    text.includes("cycle-free")
  ) {
  ...
  if (
    ...
    const isDesc = text.includes("descend");
    return (state) =>
      evaluateOrdering(state, isDesc ? "descending" : "ascending");
  }
  ...
  if (
    ...
    return (state) => evaluateRangeValidity(state);
  }

  if (
    text.includes("reference") ||
    text.includes("pointer") ||
    text.includes("valid pointer") ||
    text.includes("not null")
  ) {
    return (state) => evaluateReferenceValidity(state);
  }
  ...
  if (
    ...
    return (state, history) => evaluateMathematicalConstraints(state, history);
  }

  if (
    text.includes("conservation") ||
    text.includes("conserve") ||
    text.includes("constant total")
  ) {
    ...
    return (state) => {
      ...
      for (const ent of state.entities.values()) {
        ...
        if (seen.has(val)) {
          return {
            holds: false,
            details: `Uniqueness violated: duplicate value '${val}'`,
          };
        }
        seen.add(val);
      }
  ...
  return (state) => {
    for (const [id, ent] of state.entities.entries()) {
      if (!id || !ent.label) {
        return {
          holds: false,
          details: `Entity integrity violated: missing ID or label in state ${state.index}`,
        };
      }
    }
    for (const rel of state.relationships.values()) {
      if (!state.entities.has(rel.source) || !state.entities.has(rel.target)) {
        return {
          holds: false,
          details: `Dangling relationship '${rel.id}': (${rel.source} -> ${rel.target})`,
        };
      }
    }
    return { holds: true };
...
export class InvariantEngine {
  ...
  public static discoverInvariants(
    ...
    invariants.push({
      ...
      statement:
        "Entities must maintain persistent semantic identity across all state transitions.",
      ...
      evaluator: (state: SemanticState) => {
        for (const [id, ent] of state.entities.entries()) {
          if (!id || !ent.label) {
            return {
              holds: false,
              details: `Missing entity ID or label in state ${state.index}`,
            };
          }
        }
        return { holds: true };
    ...
    invariants.push({
      ...
      statement:
        "All relationship endpoints must reference valid, existing entities.",
      category: "relational",
      scope: "global",
      severity: "critical",
      source: "universal-core",
      predicate: (state: SemanticState) => {
        for (const rel of state.relationships.values()) {
          if (
            !state.entities.has(rel.source) ||
            !state.entities.has(rel.target)
          ) {
            return false;
          }
        }
      ...
      evaluator: (state: SemanticState) => {
        for (const rel of state.relationships.values()) {
          if (
            ...
            return {
              holds: false,
              details: `Dangling relationship '${rel.id}' references missing entity`,
            };
          }
        }
        return { holds: true };
    ...
    if (text.includes("avl")) {
      invariants.push({
        ...
        statement:
          "AVL Tree Balance: For every node N, |height(L) - height(R)| <= 1",
        category: "algebraic",
        scope: "terminal",
        severity: "critical",
        source: "domain:avl",
      ...
      invariants.push({
        ...
        statement:
          "Binary Search Tree Order: left children < node < right children",
        category: "algebraic",
        scope: "global",
        severity: "critical",
        source: "domain:bst",
    ...
    for (const rule of rules) {
      if (rule.validationMethod) {
        invariants.push({
          ...
          evaluator: (state) => ({
            holds: Boolean(rule.validationMethod?.(state)),
          }),
        });
      }
    }
  ...
  public static validateStateInvariants(
        ...
        if (inv.evaluator) {
          ...
          if (!res.holds) {
            violations.push({
              ...
              details:
                res.details ||
                `Invariant '${inv.statement}' failed in state ${state.index}`,
            });
          }
          ...
          const fallback = synthesizeConstraintEvaluator(
            inv.statement,
            inv.category,
          );
          ...
          if (!res.holds) {
            violations.push({
              ...
              stateIndex: state.index,
              details:
                res.details || `Invariant '${inv.statement}' failed validation`,
            });
          }
        }
      } catch (err: unknown) {
    ...
    const explanations = violations.map((v) =>
      explainInvariantViolation(v, state),
    );
