...
export function computeSemanticDiff(
  ...
  for (const [id, annB] of gB.annotations) {
    if (!gA.annotations.has(id)) {
      ...
      if (
        annA.text !== annB.text ||
        annA.targetEntityId !== annB.targetEntityId
      ) {
        updatedAnns.push(id);
      }
    }
  ...
  if (addedEntities.length > 0) {
    summary.push(`+${addedEntities.length} entities`);
  }
  if (removedEntities.length > 0) {
    summary.push(`-${removedEntities.length} entities`);
  }
  if (updatedEntities.length > 0) {
    summary.push(`${updatedEntities.length} entities updated`);
  }
  if (addedRels.length > 0) {
    summary.push(`+${addedRels.length} edges`);
  }
  if (removedRels.length > 0) {
    summary.push(`-${removedRels.length} edges`);
  }
  if (updatedRels.length > 0) {
    summary.push(`${updatedRels.length} edges updated`);
  }
  if (rootChanged) {
    summary.push(
      `root changed (${gA.metadata?.rootEntityId} -> ${gB.metadata?.rootEntityId})`,
    );
  }
...
export function calculateTreeDepth(graph: SceneGraph, rootId?: string): number {
  ...
  const effectiveRootId =
    rootId || graph.metadata?.rootEntityId || treeNodes[0].id;
...
export function validateTreeComplexity(
  ...
  const requestedDepthMatch = promptLower.match(
    /\b(height|depth)[-\s]+(\d+)\b/,
  );
  ...
  if (
    !isTrivialRequested &&
    depth < 2 &&
    initialState.graph.entities.size > 0
  ) {
    const treeNodesCount = Array.from(
      initialState.graph.entities.values(),
    ).filter((e) => e.primitiveType === "TreeNode").length;
...
export function validateAVLRotation(
  ...
  if (timeline.states.length < 2) {
    return {
      valid: false,
      error:
        "AVL rotation lesson must contain at least 1 transformation showing the rotation.",
    };
  }
  ...
  function getTreeChildren(graph: SceneGraph): {
    ...
    for (const rel of graph.relationships.values()) {
      const isL =
        rel.type === "left" || rel.type === "leftOf" || rel.label === "L";
      const isR =
        rel.type === "right" || rel.type === "rightOf" || rel.label === "R";
  ...
  if (
    initialTree.root &&
    finalTree.root &&
    initialTree.root === finalTree.root &&
    initialState.graph.entities.size >= 3
  ) {
    const diff = computeSemanticDiff(initialState, finalState);
    if (
      !diff.hasChanges ||
      (diff.relationshipChanges.updated.length === 0 &&
        diff.relationshipChanges.added.length === 0)
    ) {
      return {
        valid: false,
        error: `AVL ${
          isRightRotation ? "right" : "left"
        } rotation claimed, but root remained '${
          initialTree.root
        }' and parent-child edges were not restructured.`,
      };
    }
  }
  ...
  function getInOrder(
    nodeId?: string,
    tree?: { left: Map<string, string>; right: Map<string, string> },
    graph?: SceneGraph,
  ): number[] {
    if (!nodeId || !tree || !graph) {
      return [];
    }
    ...
    if (leftId) {
      seq.push(...getInOrder(leftId, tree, graph));
    }
    if (!Number.isNaN(val)) {
      seq.push(val);
    }
    ...
    if (rightId) {
      seq.push(...getInOrder(rightId, tree, graph));
    }
    return seq;
  }

  const inOrderInitial = getInOrder(
    initialTree.root,
    initialTree,
    initialState.graph,
  );
  ...
  if (
    inOrderInitial.length > 1 &&
    inOrderFinal.length === inOrderInitial.length
  ) {
    ...
    if (isInitialBST && !isFinalBST) {
      return {
        valid: false,
        error: `AVL rotation corrupted Binary Search Tree in-order invariant: [${inOrderFinal.join(
          ", ",
        )}] does not match expected sorted sequence [${initialSorted.join(
          ", ",
        )}].`,
      };
    }
  ...
  function getHeight(
    ...
    if (!nodeId || !tree || !graph || !graph.entities.has(nodeId)) {
      return 0;
    }
  ...
  for (const nodeId of finalState.graph.entities.keys()) {
    const hL = getHeight(
      finalTree.left.get(nodeId),
      finalTree,
      finalState.graph,
    );
    const hR = getHeight(
      finalTree.right.get(nodeId),
      finalTree,
      finalState.graph,
    );
    if (Math.abs(hL - hR) > 1) {
      ...
      return {
        valid: false,
        error: `AVL rotation failed to balance tree at node '${
          ent?.label || nodeId
        }': left height is ${hL}, right height is ${hR} (diff ${Math.abs(
          hL - hR,
        )} > 1).`,
      };
    }
 ...
 */
export function validateBFSTraversal(timeline: CompiledTimeline): {
  valid: boolean;
  error?: string;
} {
  if (timeline.states.length < 2) {
    return {
      valid: false,
      error:
        "BFS traversal lesson must have progressive transformations showing queue and node exploration.",
    };
  }
  ...
  if (initialFp === finalFp) {
    return {
      valid: false,
      error:
        "BFS lesson error: initial state and final state are identical. The traversal never progressed.",
    };
  }
  ...
  if (explorationProgressionCount === 0) {
    return {
      valid: false,
      error:
        "BFS traversal transformations did not alter any graph nodes, queue, or visited states.",
    };
  }
...
export function validateTransformationTimeline(
  ...
  const hasSubsequentEntities = timeline.states.some(
    (s) => s.graph && s.graph.entities.size > 0,
  );
  if (initialGraph.entities.size === 0) {
    if (!hasSubsequentEntities) {
      errors.push("Scene contains no semantic entities across any state.");
    } else {
      warnings.push(
        "Initial scene is empty (baseline state before first element arrival).",
      );
    }
  } else if (
    initialGraph.entities.size === 1 &&
    timeline.states.length <= 1 &&
    (Array.from(initialGraph.entities.values())[0]
      .id.toLowerCase()
      .includes("title") ||
  ...
  // 2. Relationship reference integrity: auto-repair dangling relationships
  for (let sIdx = 0; sIdx < timeline.states.length; sIdx++) {
    ...
    for (const relId of relsToDelete) {
      state.graph.relationships.delete(relId);
      warnings.push(
        `State ${sIdx} auto-pruned dangling relationship '${relId}'.`,
      );
      repaired = true;
    }
  }
  ...
  for (let i = 0; i < timeline.states.length - 1; i++) {
    ...
    if (!diff.hasChanges || fingerprints[i] === fingerprints[i + 1]) {
      warnings.push(
        `Transformation ${i + 1} ('${
          timeline.meta[i + 1]?.title || i + 1
        }') produced no semantic changes (identical fingerprint).`,
      );
  ...
  if (
    repaired &&
    statesToKeep.length > 1 &&
    statesToKeep.length < timeline.states.length
  ) {
  ...
  // Rule 8 & 49: Final state must differ from initial state (unless intentional cycle or auto-repaired exploration)
  if (!options?.allowCycles && finalTimeline.states.length > 1) {
    ...
    if (initialFp === finalFp) {
      // Check if intermediate exploration/traversal occurred
      let hadIntermediateProgression = false;
      for (let i = 1; i < finalTimeline.states.length - 1; i++) {
        if (computeSemanticFingerprint(finalTimeline.states[i]) !== initialFp) {
          hadIntermediateProgression = true;
          break;
        }
      }

      if (hadIntermediateProgression) {
        warnings.push(
          "Lesson returned to initial state after exploration; auto-applying terminal completion state.",
        );
        repaired = true;
        const lastState = finalTimeline.states[finalTimeline.states.length - 1];
        if (lastState && lastState.graph.entities.size > 0) {
          const firstEnt = lastState.graph.entities.values().next().value;
          if (firstEnt) {
            firstEnt.properties = {
              ...(firstEnt.properties || {}),
              highlight: "completed",
        ...
        warnings.push(
          "Final scene state matched initial state; auto-applying completion highlight to final state.",
        );
        repaired = true;
        const lastState = finalTimeline.states[finalTimeline.states.length - 1];
        if (lastState && lastState.graph.entities.size > 0) {
    ...
    if (typeof inv.check === "function") {
      for (let sIdx = 0; sIdx < finalTimeline.states.length; sIdx++) {
        const dummyConceptModel: any = {
          ...
          entities: Array.from(
            finalTimeline.states[sIdx].graph.entities.values(),
          ),
          relationships: Array.from(
            finalTimeline.states[sIdx].graph.relationships.values(),
          ),
        };
        const pass = inv.check(
          {
            ...
            activeEntityIds: Array.from(
              finalTimeline.states[sIdx].graph.entities.keys(),
            ),
            activeRelationshipIds: Array.from(
              finalTimeline.states[sIdx].graph.relationships.keys(),
            ),
          },
          dummyConceptModel,
        );
        if (!pass) {
          warnings.push(
            `State ${sIdx} invariant notice '${inv.id}': ${inv.description}`,
          );
        }
      }
    }
  ...
  // Target Values Conservation Check:
  // If prompt explicitly declared multiple numeric values (e.g. 30, 20, 10, 25, 28, 27, 50, 60, 55),
  // verify all numbers exist in the final state!
  const requestedValues = Array.from(prompt.matchAll(/\b\d+\b/g)).map((m) =>
    Number(m[0]),
  );
  if (requestedValues.length >= 3 && finalTimeline.states.length > 0) {
    const finalGraph =
      finalTimeline.states[finalTimeline.states.length - 1].graph;
    const finalValSet = new Set<number>();
    for (const ent of finalGraph.entities.values()) {
      const v = Number(ent.value ?? ent.label);
      if (!isNaN(v)) {
        finalValSet.add(v);
      }
    }
    const missing = requestedValues.filter((v) => !finalValSet.has(v));
    if (missing.length > 0) {
      warnings.push(
        `Final scene state is missing requested values: [${missing.join(
          ", ",
        )}].`,
      );
    }
