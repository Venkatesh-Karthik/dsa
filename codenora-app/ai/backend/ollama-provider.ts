...
export class OllamaTeachingProvider implements TeachingProvider {
  ...
  async generateTeachingResponse(
    ...
    throw new Error(
      `Ollama (${
        this.model
      }) generated invalid Visual DSL: ${retryValidated.errors.join("; ")}`,
    );
  }
      ...
      if (err instanceof Error && err.name === "AbortError") {
        throw new Error(
          `Ollama request timed out after ${
            this.timeoutMs / 1000
          }s. Check if Ollama is running and model '${this.model}' is loaded.`,
        );
      }
      throw err;
    } finally {
  ...
  private async callOllamaGenerateFallback(
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `Ollama generate error (${response.status}): ${
            errorText || response.statusText
          }`,
        );
      }
