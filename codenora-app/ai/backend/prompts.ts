...
export function formatTeachingUserPrompt(request: TeachingRequest): string {
  ...
  if (request.context) {
    ...
    if (
      ...
      parts.push(
        `\nACTIVE LESSON STATE:\n- Lesson Topic: "${
          al.topic ?? "Current Topic"
        }"\n- Current Step: Step ${(al.currentStepIndex ?? 0) + 1} of ${
          al.totalSteps ?? 1
        } ("${
          al.stepTitle ?? ""
...
DO NOT write any explanation, reasoning, or prose outside the JSON.
DO NOT wrap the JSON in markdown code fences (no \`\`\` or \`\`\`json).
If you must reason internally, wrap it ONLY inside <think>...</think> tags BEFORE the JSON.
