...
describe("Cognora 5.0 — Universal Visual Reasoning Engine", () => {
  ...
  describe("1. Visual Capability Registry & Dynamic Affordances", () => {
    it("contains foundational capabilities across all required categories", () => {
      ...
      const mathematical =
        VisualCapabilityRegistry.getByCategory("mathematical");
    ...
    it("dynamically resolves capabilities from roles, types, and labels without topic hardcoding", () => {
      // Endpoints & Actors
      expect(
        VisualCapabilityRegistry.resolveMatchingCapability("client", "Endpoint")
          .id,
      ).toBe("Client");
      expect(
        VisualCapabilityRegistry.resolveMatchingCapability("server", "Host").id,
      ).toBe("Server");
      expect(
        VisualCapabilityRegistry.resolveMatchingCapability(
          "actor",
          "Participant",
        ).id,
      ).toBe("Actor");
      ...
      // Tabular & Records
      expect(
        VisualCapabilityRegistry.resolveMatchingCapability("table", "Dataset")
          .id,
      ).toBe("Table");
      expect(
        VisualCapabilityRegistry.resolveMatchingCapability("record", "Row").id,
      ).toBe("Record");
      ...
      expect(
        VisualCapabilityRegistry.resolveMatchingCapability("ray", "Beam").id,
      ).toBe("TrajectoryRay");
      expect(
        VisualCapabilityRegistry.resolveMatchingCapability(
          "equation",
      ...
      const mockModel: AuthoritativeSemanticModel = {
        ...
        world: {
          entities: [
            {
              id: "acc-a",
              label: "Account A (Balance: $500)",
              type: "Table",
              semanticRole: "table",
            },
            {
              id: "acc-b",
              label: "Account B (Balance: $200)",
              type: "Table",
              semanticRole: "table",
            },
            {
  ...
  describe("3. Visual Composition Planning", () => {
    it("dynamically plans two-party interaction composition from topology", () => {
      const model: AuthoritativeSemanticModel = {
        id: "two-party-model",
        problem: {
          concept: "Protocol Flow",
          question: "Client to server request",
          intent: "EXPLAIN",
          objective: "Flow",
        } as any,
        world: {
    ...
    it("dynamically plans circular cycle composition for closed-loop graphs", () => {
      const model: AuthoritativeSemanticModel = {
        id: "cycle-model",
        problem: {
          concept: "Energy Cycle",
          question: "Closed biological cycle",
          intent: "EXPLAIN",
          objective: "Cycle",
        } as any,
        world: {
    ...
    it("dynamically plans composite composition (tabular + flow) for transactional ledgers", () => {
      const model: AuthoritativeSemanticModel = {
        ...
        problem: {
          concept: "Ledger Transaction",
          question: "Ledger update with state validation",
          intent: "EXPLAIN",
          objective: "Tx",
        } as any,
        world: {
  ...
  describe("4. Relationship & Connector Planning", () => {
    it("enforces real connectors with endpoint bindings, routing, and proper directions", () => {
      const model: AuthoritativeSemanticModel = {
        id: "rel-model",
        problem: {
          concept: "Message Flow",
          question: "Sender transmits packet to receiver",
          intent: "EXPLAIN",
          objective: "Transmit",
        } as any,
        world: {
  ...
  describe("5. Spatial Intent Planning", () => {
    it("formulates natural reading direction and hierarchical ranks", () => {
      ...
      const elPlan = VisualElementPlanner.plan(model, evPlan, compPlan);
      const relPlan = RelationshipPlanner.plan(model, elPlan, evPlan);
      const spatialPlan = SpatialIntentPlanner.plan(
        model,
        compPlan,
        elPlan,
        relPlan,
      ...
      const rFetch = spatialPlan.ranks.find((r) =>
        r.entityIds.includes("stage-fetch"),
      );
      const rDecode = spatialPlan.ranks.find((r) =>
        r.entityIds.includes("stage-decode"),
      );
      const rExecute = spatialPlan.ranks.find((r) =>
        r.entityIds.includes("stage-execute"),
      );
  ...
  describe("6. Visual Transformation Planning", () => {
    it("generates structured pedagogical step plans answering the 5 educational questions", () => {
      const model: AuthoritativeSemanticModel = {
        id: "trans-model",
        problem: {
          concept: "State Switch",
          question: "Toggle active state",
          intent: "EXPLAIN",
          objective: "Toggle",
        } as any,
        world: {
        ...
        states: [
          createSemanticState(0, "Initial", {
            entities: new Map([
              [
                "node-1",
                createSemanticEntity("node-1", "GenericEntity", "Node 1", {
                  state: "idle",
                }),
              ],
            ]),
      ...
      const elPlan = VisualElementPlanner.plan(model, evPlan, compPlan);
      const relPlan = RelationshipPlanner.plan(model, elPlan, evPlan);
      const transPlan = VisualTransformationPlanner.plan(
        model,
        elPlan,
        relPlan,
      );
  ...
  describe("7. Visual Quality Validator & Auto-Repair", () => {
    it("evaluates visual quality and automatically repairs missing connectors or spacing issues", () => {
      ...
      expect(
        plan.qualityReport?.dimensions.semanticCoverage,
      ).toBeGreaterThanOrEqual(70);
      expect(
        plan.qualityReport?.dimensions.relationshipVisibility,
      ).toBeGreaterThanOrEqual(70);
      expect(
        plan.qualityReport?.dimensions.collisionSafety,
      ).toBeGreaterThanOrEqual(70);
    });
  });
  ...
  describe("8. Master Timeline Compilation & State Generation", () => {
    it("compiles authoritative semantic models directly into complete SceneStates using VisualReasoningEngine", () => {
      ...
      expect(s1.graph.entities.get("pc")?.value).toBe(
        "Program Counter (0x008)",
      );
  ...
  describe("9. Six Real-World Acceptance Test Concepts", () => {
    it("Acceptance 1: TCP 3-Way Handshake with real packets, endpoints, and elbowed connectors", () => {
      ...
      expect(timeline.visualPlan?.compositionPlan.primaryStrategy).toBe(
        "interaction",
      );
    });
          ...
          steps: [
            {
              ...
              explanation:
                "Debit exceeding balance triggers constraint violation",
              stateType: "failure",
    ...
    it("Acceptance 3: CPU Instruction Cycle (Fetch, Decode, Execute, Writeback)", () => {
      ...
      expect(timeline.visualPlan?.spatialIntentPlan.readingDirection).toBe(
        "left_to_right",
      );
    });
    ...
    it("Acceptance 4: Projectile Motion (Physics trajectory, apex, vectors)", () => {
          ...
          steps: [
            {
              ...
              operations: [
                { type: "update_entity", entityId: "apex", state: "active" },
              ],
            },
          ],
        },
      ...
      expect(timeline.states.length).toBeGreaterThanOrEqual(2);
      expect(
        timeline.visualPlan?.qualityReport?.overallScore,
  ...
  describe("10. 100% Local Playback & Inspector Guarantee", () => {
    it("simulates complete local navigation (Next, Prev, Seek, Inspect) with ZERO network/AI calls", async () => {
      ...
      const firstEntityId = Array.from(
        timeline.states[0].graph.entities.keys(),
      )[0];
