...
describe("Universal Visual Spatial Intelligence - Bounding Box Derivation", () => {
  it("derives content-aware bounds based on primitive type and text length", () => {
    ...
    const containerBounds = deriveEntityBounds(containerEntity, {
      x: 300,
      y: 300,
    });
...
describe("Universal Visual Spatial Intelligence - Multi-Rank DAG Layout (e.g. Rainbow / Complex Concept)", () => {
  it("computes non-overlapping multi-rank DAG layout with non-zero horizontal span", () => {
    ...
    addRelationship(graph, {
      ...
      type: "connects",
      sourceEntityId: "sun",
      targetEntityId: "droplet",
    });
    addRelationship(graph, {
      id: "r2",
      type: "connects",
      sourceEntityId: "droplet",
      targetEntityId: "refraction",
    });
    addRelationship(graph, {
      id: "r3",
      type: "connects",
      sourceEntityId: "droplet",
      targetEntityId: "reflection",
    });
    addRelationship(graph, {
      id: "r4",
      type: "connects",
      sourceEntityId: "droplet",
      targetEntityId: "dispersion",
    });
    addRelationship(graph, {
      id: "r5",
      type: "connects",
      sourceEntityId: "refraction",
      targetEntityId: "spectrum",
    });
    addRelationship(graph, {
      id: "r6",
      type: "connects",
      sourceEntityId: "reflection",
      targetEntityId: "spectrum",
    });
    addRelationship(graph, {
      id: "r7",
      type: "connects",
      sourceEntityId: "dispersion",
      targetEntityId: "spectrum",
    });
    addRelationship(graph, {
      id: "r8",
      type: "connects",
      sourceEntityId: "spectrum",
    ...
    for (let i = 0; i < entityBoundsList.length; i++) {
      for (let j = i + 1; j < entityBoundsList.length; j++) {
        ...
        expect(
          ...
          `Entities "${a.id}" and "${
            b.id
          }" collide unexpectedly: ${JSON.stringify(
            a.bounds,
          )} vs ${JSON.stringify(b.bounds)}`,
        ).toBe(false);
      }
    }
  });
...
describe("Universal Visual Spatial Intelligence - Cycle Layout Separation", () => {
  it("expands radius for cyclic relationships so nodes do not overlap", () => {
    ...
    addRelationship(graph, {
      id: "c1",
      type: "connects",
      sourceEntityId: "stateA",
      targetEntityId: "stateB",
    });
    addRelationship(graph, {
      id: "c2",
      type: "connects",
      sourceEntityId: "stateB",
      targetEntityId: "stateC",
    });
    addRelationship(graph, {
      id: "c3",
      type: "connects",
      sourceEntityId: "stateC",
      targetEntityId: "stateD",
    });
    addRelationship(graph, {
      id: "c4",
      type: "connects",
      sourceEntityId: "stateD",
      targetEntityId: "stateE",
    });
    addRelationship(graph, {
      id: "c5",
      type: "connects",
      sourceEntityId: "stateE",
      targetEntityId: "stateA",
    });
...
describe("Universal Visual Spatial Intelligence - Non-Occluding Annotation Placement", () => {
  it("places annotations into a collision-free candidate slot when obstacles exist", () => {
    const anchorBounds: LayoutBounds = {
      x: 200,
      y: 200,
      width: 80,
      height: 60,
    };
    const annotationBounds: LayoutBounds = {
      x: 0,
      y: 0,
      width: 140,
      height: 50,
    ...
    const obstacleAbove: LayoutBounds = {
      x: 190,
      y: 100,
      width: 100,
      height: 60,
...
describe("Universal Visual Spatial Intelligence - Connector Robustness & Obstacle Avoidance", () => {
  ...
  it("detects intermediate obstacles along the direct path between source and target", () => {
    ...
    const detected = detectObstaclesBetween(fromBounds, toBounds, [
      obstacleRecord,
    ]);
