...
describe("TransformationTimeline - AVL Rotations", () => {
    ...
    transformations: [
      {
        ...
        explanation:
          "Rotate right around node 30 so node 20 becomes the new root.",
        operations: [
          {
            type: "create_tree",
            id: "avl-tree",
      ...
      },
      {
        ...
        explanation:
          "All balance factors are now 0. Tree is perfectly balanced.",
        operations: [
          {
            type: "highlight",
            target: "avl-tree-n20",
  ...
  it("compiles into complete SceneStates preserving stable node IDs", () => {
    ...
    expect(s1.graph.entities.get("avl-tree-n20")?.properties?.highlight).toBe(
      "success",
    );

    // Relationships in State 1 reflect rotation: 20 -> 10 and 20 -> 30
    expect(s1.graph.relationships.has("edge-avl-tree-n20-avl-tree-n10")).toBe(
      true,
    );
    expect(s1.graph.relationships.has("edge-avl-tree-n20-avl-tree-n30")).toBe(
      true,
    );
    // Old relationship 30 -> 20 is gone
    expect(s1.graph.relationships.has("edge-avl-tree-n30-avl-tree-n20")).toBe(
      false,
    );
    ...
    expect(s2.graph.entities.get("avl-tree-n20")?.properties?.highlight).toBe(
      "primary",
    );
  });
  ...
  it("reconciles target states with zero missing nodes or orphaned arrows", () => {
    ...
    const resPrev = reconcileSceneState(
      timeline.states[0],
      res1.elements,
      timeline.lessonId,
    );
    ...
    const fridgeGraph = compileGenericConceptGraph(
      ...
      [
        ...
        },
        {
          ...
          to: "evaporator",
          label: "Cold low pressure liquid",
        },
